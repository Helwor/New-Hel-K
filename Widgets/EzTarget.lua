...
local function Evaluate(type, id, engineCmd)
	...
	if type == 'unit' then
		...
		if unit then
			...
			onSelf = isMine and not sel[2] and (
				traced == sel[1]
	...
	if not (alt or --[[shift or--]] ctrl) then
		...
		if selContext.hasControllableRepairer then
			if not padToRearm then
				...
				elseif v.defaultCmd == CMD_REPAIR or v.moddedCmd == CMD_REPAIR then
...
do --- EzTarget ---
	...
	local spWorldToScreenCoords = Spring.WorldToScreenCoords
	local spGetGroundHeight = Spring.GetGroundHeight
	local function Transform(mx, my, throughWater, margin, waterLevel, recursion)
		margin = 8
		waterLevel = waterLevel or spGetWaterLevel(0,0)
		local nature, pos = spTraceScreenRay(mx, my, true, false, true, true, waterLevel) -- assume the water level is uniform, but some map can have different water level over time
		local ori_pos = pos
		local gpos
		local clamped = false
		...
		if nature == "sky" then -- off map
			-- Echo("pos[4], pos[5], pos[6] is ", pos[4], pos[5], pos[6])
			local edgeX, edgeZ, sX, sZ
			-- Echo(pos[2], " edgeX, edgeZ, sX, sZ ", select(3, Clamp(pos[1], pos[3], margin or 1)))
			local ori_mx, ori_my = mx, my
			local x, y, z = pos[4], pos[5], pos[6] -- get the mouse pos at plane of water level
			local wx, wy, wz = x, y, z -- water plane pos
			-- Echo("wx, wy, wz is ", wx, wy, wz, 'p1, p2, p3', pos[1], pos[2], pos[3])
			x, z, edgeX, edgeZ, sX, sZ = Clamp(x, z, margin or 1) -- clamp at the edge of map

			-- Echo("edgeX is ", edgeX, mapSizeX)
			mx, my = clampscreen(spWorldToScreenCoords(x, y, z)) -- pick up the new screen position after clamping
			 -- trace pos this time at ground height
			nature, pos = spTraceScreenRay(mx, my, true, false, true, true)
			local done = false
			if nature == 'sky' then
				clamped = true
				-- nature can be sky if ground is underwater or between waterLevel and map ground surface
				local gy = spGetGroundHeight(x, z)
				if throughWater then -- case: in sky because ground is underwater
					
					y = gy -- update the ground height after translation
					local _mx, _my = clampscreen(spWorldToScreenCoords(x, y, z))
					local _nature, _pos = spTraceScreenRay(_mx, _my, true, false, true, true)
					DBG[#DBG+1] = {_pos[1], _pos[2], _pos[3]}
					-- we check original screen pos on plane of ground height
					_, gpos = spTraceScreenRay(ori_mx, ori_my, true, false, true, true, gy)
					-- recheck edge, mouse pos is out of map but the water surface of the pos is not
					--				or edge might be wrong on corner as it was evaluated through water plane instead of ground plane
					_, _, edgeX, edgeZ, sX, sZ = Clamp(gpos[4], gpos[6], margin or 1)	
					x, y, z = edgeX or gpos[4], gpos[5], edgeZ or gpos[6]
					DBG[#DBG+1] = {x, y, z}


					--
					_mx, _my = clampscreen(spWorldToScreenCoords(x, y, z))
					nature, _pos = spTraceScreenRay(_mx, _my, true, false, true, true)


					if nature == 'sky' then
						local tries = 0
						gy = spGetGroundHeight(gpos[4], gpos[6])
						y = gy
						repeat
							tries = tries + 1
							-- we're not pointing to the ground yet
							-- redo same operation, pick the ground height, reapply the trace this time on that lower height plane, translate back the point to the edge

							_, gpos = spTraceScreenRay(_mx, _my, true, false, true, true, gy)
							x, y, z = edgeX or gpos[4], gpos[5], edgeZ or gpos[6]
							DBG[#DBG+1] = {x, y, z}
							...
							gy = spGetGroundHeight(gpos[4], gpos[6]) 
							y = gy
							if math.abs(_pos[2] - gy) < 10 then
								if nature ~= 'sky' then
									-- Echo('found ground')
									x, z = edgeX or _pos[1], edgeZ or _pos[3]
								else
									-- Echo('close but in water')
								end
						...
						until nature ~= 'sky' or tries == 3
					end
					if not done and (math.abs(_pos[2] - gy) < 10 or nature == 'sky') then
						if nature ~= 'sky' then
							x, z = edgeX or _pos[1], edgeZ or _pos[3]
						end
						y = spGetGroundHeight(x, z)
			...
			if not done and nature == 'ground' then -- in case we fall on ground
				local _nature, _pos = nature, DBG[#DBG] or pos
				x, y, z = edgeX or _pos[1], _pos[2], edgeZ or _pos[3]
				local _mx, _my
				local gy = spGetGroundHeight(x,z)

				if math.abs(y - gy) < 10 then
					DBG[#DBG+1] = {x, gy, z}
					done = true
				end
				if not done then
					...
					repeat 
						...
						_mx, _my = clampscreen(spWorldToScreenCoords(x, y, z)) -- pick up the new screen position after clamping
						_nature, _pos = spTraceScreenRay(_mx, _my, true, false, true, true)
						y = spGetGroundHeight(x,z)
						if math.abs(_pos[2] - y) < 10 or _nature == 'sky' then
							if _nature ~= 'sky' then
								x, z = edgeX or _pos[1], edgeZ or _pos[3]
							end
							DBG[#DBG+1] = {x, y, z}
			...
			-- Echo(nature, "pos[2] is ", pos[2], pos[5], spGetGroundHeight(pos[1], pos[3]))
		end
	...
	EzTarget = function(getTarget, getSelect, wantAllied)
		...
		if clamp then
			...
			-- Echo("center[1], center[3] is ", center[1], center[3])
			-- center[2] = spGetGroundHeight(center[1], center[3])
		else
			local nature
		...
		if Debug.EZ() then
			...
			for i,v in ipairs(DBG) do
				local mx, my = spWorldToScreenCoords(v[1], v[2], v[3])
				drawCircle[-i] = {mx, my, 0, 25 - 3*i, someIgnored and orange or white,0.35,false,true,false,true}
			end				
...
function widget:Initialize()
	...
	if WG.ClampScreenPosToWorld then
		...
		Echo('EzTarget is not using WG.ClampScreenPosToWorld')
