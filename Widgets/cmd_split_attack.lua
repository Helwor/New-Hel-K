...
local subjectDefID = {}
local bomberDefID = {}
for defID in pairs(UnitDefs) do
	subjectDefID[defID] = false
end
local misc = {
	cloaksnipe = true,
	gunshipemp = true,
...
for defID, def in pairs(UnitDefs) do
	...
	elseif misc[name] or name:find('arty') or name:find('raid') or name:find('skirm') or name:find('riot') or name:find('bomb') or name:find('assault') then
		subjectDefID[defID] = true
	end
end
...
options = {}
options_order = {'active', 'unit_types'}
...
options.unit_types = {
	name = 'Unit Types Enabled',
	type = 'table',
	desc = 'Choose unit types that can Split Attack',
	value = subjectDefID,
	isCheckboxList = true,
	noRemove = true,
	preTreatment = function(copy)
		local names = {}
		local uds = UnitDefs
		for defID, bool in pairs(copy) do
			local name = uds[defID].name
			names[name] = bool
			copy[defID] = nil
		end
	...
	postTreatment = function(t)
		local udnames = UnitDefNames
		local defIDs = {}
		for name, bool in pairs(t) do
			local defID = udnames[name].id
			defIDs[defID] = bool
		end
		for defID, bool in pairs(defIDs) do
			t[defID] = bool
		end
	end
}
...
DistributeOrders = function()
	-- distributing orders
	local debugOrdering = Debug.ordering()
	local debugGround = Debug.ground_target() and not debugOrdering
	local debug = debugGround or debugOrdering
	...
	for defid, units in pairs(WG.selectionDefID or spGetSelectedUnitsSorted()) do
		...
		if attacking and subjectDefID[defid] then
			...
			for i,id in ipairs(units) do
				if not bomberDefID[defid] or spGetUnitRulesParam(id,'noammo') ~= 1 then -- the bomber can attack
					n = n + 1
					sel[n] = id
...
MapTarget = function(tx, ty, tz, tid, map, cmd) -- make unique points
	...
	-- if not DONE then
	-- 	-- DONE = true
	-- 	Echo('in map:', next(map), next(map) and next(select(2, next(map))), next(select(2, next(map))) and select(2, next(select(2, next(map)))).id)
