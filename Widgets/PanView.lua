function widget:GetInfo()
	return {
		name      = "Pan View",
		desc      = "Pan view by holding space and left click down and dragging mouse (and much more)",
		author    = "Helwor",
		version   = "v1",
		date      = "mid 2021",
...
-- CONFIG
	...
	ignoreMiniMap = false,
	clamp = 300, -- how much the center view can go out of the map for COFC -- false for no limit
	forceOnFight = true,
...
options_order = {
	...
	'lbl_other',
	'late_space_tol','space_info_height','suppress_wreckage_info',
	'stay_at_zoomed_height',
...
options = { -- TODO finish adding options
	...
	smoothness_zoom = {
		name = 'Smoothness On Zoom',
		desc = '',
		type = 'number',
		min = 0, max = 1, step = 0.001,
		value = cfg.smoothness_zoom,
		OnChange = function(self)
			cfg[self.key] = self.value
			-- v.clampValue, v.clampValue  = mapSizeX*(cfg.clamp-1), mapSizeZ*(cfg.clamp-1)
			-- v.clampValue = math.min(mapSizeX,mapSizeZ)*(cfg.clamp-1)
	...
	smoothness_zoom_back = {
		name = 'Smoothness On Zoom Back',
		desc = '',
		type = 'number',
		min = 0, max = 1, step = 0.001,
		value = cfg.smoothness_zoom_back,
		OnChange = function(self)
			cfg[self.key] = self.value
			-- v.clampValue, v.clampValue  = mapSizeX*(cfg.clamp-1), mapSizeZ*(cfg.clamp-1)
			-- v.clampValue = math.min(mapSizeX,mapSizeZ)*(cfg.clamp-1)
	...
	amplitude = {
		name = 'Amplitude',
		desc = 'how much the mouse movement impact the panning.',
		type = 'number',
		min = 0.2, max = 5, step = 0.02,
		value = cfg.amplitude,
		OnChange = function(self)
			cfg[self.key] = self.value
		end,
	...
	drift = {
		name = 'Drift at mouse click',
		desc = 'how much the camera drift toward the mouse click.',
		type = 'number',
	...
	stay_at_zoomed_height = {
		name = 'Stay At Zoomed Height',
		desc = 'Stay at zoomed height on release.',
		type = 'bool',
		value = cfg.stay_at_zoomed_height,
		OnChange = function(self)
			cfg[self.key] = self.value
		end
	},
...
for key,value in pairs(cfg) do -- add some raw options for testing, might not be useful/used
	if not (options[key] or exception[key]) then
		...
		if type(value) == 'number' then
			options[key].min = value*0.3
			options[key].max = value*3
...
local function SetCamViewPos(cs,pos, mul)
	if mul and mul ~= 1 then
		local oldPos = cs.viewPos
		pos[1] = oldPos[1] + (pos[1] - oldPos[1]) * mul
		pos[3] = oldPos[3] + (pos[3] - oldPos[3]) * mul
...
local function Locking(cs,transTime, mx, my)
	...
	if transTime then
		interp_zoom = {timer = Spring.GetTimer(), transTime = transTime}
	end
...
local function SlideCamera(cs,dx,dy)
	if (cs.mode == 1) then
		...
		local camVecs = Cam.vecs
		local cf = camVecs.forward
		local len = sqrt((cf[1]^2) + (cf[3]^2))
		local dfx = cf[1] / len
		local dfz = cf[3] / len
		local cr = camVecs.right
		local len = sqrt((cr[1]^2) + (cr[3]^2))
		local drx = cr[1] / len
		local drz = cr[3] / len
		...
		if cfg.follow_ground then
			cs.py = cs.py + (cs.viewPos[2] - prev_gy) / 3 --  / 3 smoothen it
		end
...
function widget:MousePress(mx, my, button, fake)
	local _, _, lmb, mmb, rmb = spGetMouseState()
	unit = false
	...
	if button==1  then
		...
		if not keyGood then
			checkForMeta = not (v.active or meta or alt or ctrl or shift) and cfg.late_space_tol > 0
			...
			elseif cs.mode~=1 and cs.mode~=4 then
				...
				if not reason then
						...
						if not v.active then
							if cfg.drift ~= 0 then
								...
								if pos then
									...
									SetCamViewPos(cs,pos, cfg.drift)
					...
					v.this_smoothness = v.zooming_out or cfg.smoothness
...
local function Move(mx,my,dx,dy)
	...
	local time = os.clock()
	if cfg.wait_before_panning then
		if time-v.started_time < cfg.wait_before_panning then
			return
		else
	...
	-- end
	if interp_zoom.timer then
		local now = Spring.GetTimer()
		local diffTime = Spring.DiffTimers(now, interp_zoom.timer)
		local transTime = interp_zoom.transTime
		if diffTime >= transTime then
			interp_zoom.timer = false
			v.this_smoothness = cfg.smoothness
		else
			-- mix initial zoom trans with move trans
			local per = diffTime / transTime
			v.this_smoothness = transTime * (1-per) + cfg.smoothness * per
		end
	end
...
function widget:MouseRelease(mx, my, button, fake)
	...
	if not new_height and v.height_backup and not cfg.stay_at_zoomed_height then
