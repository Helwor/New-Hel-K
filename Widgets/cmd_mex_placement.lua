...
-- local function GetMiniMapVisibleEnough()
-- 	local l,b,r,t, mini, maxi = spGetMiniMapGeometry()
-- 	Echo(math.round(os.clock()),"l,t,r,b, mini, maxi is ", l,t,r,b, mini, maxi, '=>>>',not mini and r-l > 40 and t-b > 40)
-- 	return not mini and r-l > 40 and t-b > 40
-- end
include("keysym.lua")
...
if dbg then
	...
	if f and f.SpiralSquare then
		SpiralSquare = f.SpiralSquare
	else
		dbg = false
	end
end
...
local function Distance2(x1,z1,x2,z2) -- ~30% faster
	local dis = (x1-x2)^2 + (z1-z2)^2
	return dis
end


local function GetClosestMetalSpot2(x, z) --it's actually much worse in perf but keeping the debugging for later
	local bestSpot
	...
	local byPos = WG.metalSpotsByPos
	local spots = WG.metalSpots
	local findings, f, foundLayer = {}, 0, false
	local dists = dbg and {}
	local find = function(layer,offx,offz)
		local t = byPos[x+offx]
		local found = t and t[z+offz]
		if foundLayer and layer > foundLayer+foundLayer/3 then
			return true
		end
		if found then
	...
	local layers = 10000
	SpiralSquare(layers,16,find,0)
	if f > 0 then
		bestIndex = findings[1]
		bestSpot = spots[bestIndex]
		bestDist = Distance(_x, _z, bestSpot.x, bestSpot.z)
		if dbg then
			dists[#dists+1] = bestIndex .. ':' .. bestDist
		end
		-- Echo("dist is ", bestIndex,bestDist)
		for i = 2, f do
			local index = findings[i]
			local spot = spots[index]
			local dist = Distance(_x,_z, spot.x, spot.z)
			if dbg then
				dists[#dists+1] = index .. ':' .. dist
			end
	...
	return bestSpot, sqrt(bestDist), bestIndex, findings, dists
end
local function GetClosestMetalSpot(x, z) --is used by single mex placement, not used by areamex
	...
	if dbg then
		t2 = Spring.GetTimer()
		t1 = Spring.DiffTimers(t2, t1)
		local bestDist2, bestIndex2, findings, dists = select(2,GetClosestMetalSpot2(x, z))
		t2 = Spring.DiffTimers(Spring.GetTimer(),t2)
		G_T1 = (G_T1 or 0) + t1
		G_T2 = (G_T2 or 0) + t2
		G_COUNT = (G_COUNT or 0) + 1
		if G_T1 > 0.1 or G_T2 > 0.1 then
			Echo('Performance New GetClosestMetalSpot #' .. G_COUNT, G_T1, G_T2,  G_T2 / G_T1)
			G_COUNT, G_T1, G_T2 = 0, 0, 0
		end
		findings.best = bestIndex2
		if bestIndex2~=bestIndex and bestDist ~= bestDist2 then
			Echo('WRONG','best',bestIndex, bestDist, 'me:',bestIndex2,bestDist2,'=>',table.concat(dists,','),'time:' .. math.round(os.clock())..'.sec')
		else
			-- Echo('ok best:' ..(bestIndex or 'none'))
		end
		FINDINGS = findings
	end
	return bestSpot, bestDist, bestIndex
end

-- local function MapMexes()
-- 	local map = setmetatable({},{__index = function(self,k) rawset(self,k,{}) return self[k] end})
-- 	for i = 1, #WG.metalSpots do
-- 		local spot = WG.metalSpots[i]
-- 		map[spot.x][spot.z] = spot
-- 	end
-- 	setmetatable(map, nil)
-- 	WG.metalSpotsMap = map
-- end

-- local FastMapper = function(map, x,z, steplevel)
-- 	-- local mindef = 16
-- 	-- local levels, nlevels = {}, 5
-- 	-- local max = math.max(x,z)
-- 	-- local mt = {__index = function(self,k) rawset(self,k,{}) return self[k] end}
-- 	-- for i = 1, nlevels do
-- 	-- 	local 
-- 	-- 	levels[i] = setmetatable({}, mt)
-- 	-- 	levels[i].def = math.max()
...
-- 	-- for i = 1, nlevels do
-- 	-- 	levels[i] = setmetatable({}, mt)
-- 	-- 	setmetatable(levels[i], mt)
-- 	-- end

-- 	-- steplevel = steplevel or math.max(x,z) / 16 / 7
...
local function TestMathSpeed(fn1, fn2, attempts)
	local spGetTimer = Spring.GetTimer
	local spDiffTimers = Spring.DiffTimers
	local r, n = math.random, 10000
	local result = {}
	for i = 1, attempts or 10 do

		local rx1, rx2, rz1, rz2 = r(n), r(n), r(n), r(n)
		local t1 = spGetTimer()
		for i = 1, 100000 do
			fn1(rx1, rx2, rz1, rz2)
		end
		t1 = spDiffTimers(spGetTimer(), t1)
		local t2 = spGetTimer()
		for i = 1, 100000 do
			fn2(rx1, rx2, rz1, rz2)
		end
		t2 = spDiffTimers(spGetTimer(), t2)
		result[i] = ('d1:%.3f, d2:%.3f => %.3f'):format(t1, t2, t2/t1)
	end
	Echo(table.concat(result,'\n'))
end
...
local function PlaceSingleMex(bx, bz, facing, options)
	...
	if closestSpot then
		...
		for i = 1, #units do
			...
			if unitDefID and mexDefIDs[unitDefID] then
...
function widget:UnitEnteredLos(unitID, teamID, forAllyTeam, unitDefID)
...
local function CheckEnemyMexes(spotID, spotM)
...
local function CheckTerrainChange(spotID, spot)
	local x = spot.x
	local z = spot.z
...
local function CheckAllTerrainChanges()
	...
	for i = 1, #metalSpots do
		...
		CheckEnemyMexes(i, spot)
		CheckTerrainChange(i, spot)
	end
end
...
function widget:GameFrame(n)
	pregame = false
	if (n % 25) ~= 0 or not WG.metalSpots then
		return
	end
...
function widget:DrawWorld()
	...
	if WG.metalSpots and (isMexCmd or pregame or ((WG.showeco or WG.showeco_always_mexes) and WG.selectionEntirelyCons) or CMD_AREA_MEX == cmdID or CMD_AREA_TERRA_MEX == cmdID) then
		...
		if closestSpot and wantShow and IsSpotBuildable(index) then
			...
			glUnitShape(mexDefID, myTeamID, false, true, false)
			glPopMatrix()
		...
		if dbg and FINDINGS then
			local bface = spGetBuildFacing()
			local function drawindex(index)
				local spot = WG.metalSpots[index]
				if not isMexCmd then
					bx, by, bz = pos[1], pos[2], pos[3]
				end
				...
				local height = spGetGroundHeight(spot.x,spot.z)
				height = height > 0 and height or 0

				glDepthTest(false)

				glLineWidth(1.49)
				if index == FINDINGS.best and FINDINGS.best == closestSpot.index then
					glColor(0.5, 1, 0, 0.5)
				elseif index == FINDINGS.best then
					glColor(1, 0, 0, 0.8)
				elseif spot == closestSpot then
					glColor(1, 1, 0, 0.5)
				else
					glColor(1, 1, 1, 0.5)
				end
				glBeginEnd(GL_LINE_STRIP, DoLine, bx, by, bz, spot.x, height, spot.z)
				glLineWidth(1.0)
				...
				-- glColor(1, 1, 1, 0.5)
				glPushMatrix()
				glTranslate(spot.x, height, spot.z)
				glRotate(90 * bface, 0, 1, 0)
				glUnitShape(mexDefID, myTeamID, false, true, false)
				glPopMatrix()
			...
			for i, index in ipairs(FINDINGS) do
				drawindex(index)
			end
			if not FINDINGS[1] and closestSpot then
				drawindex(closestSpot.index)
			end
		end
	end
...
function widget:DefaultCommand(cmdType, id)
	if autoMex and mexSpotToDraw and not cmdType and (spTestBuildOrder(primaryMexDefID, mexSpotToDraw.x, 0, mexSpotToDraw.z, 0) > 0) then
