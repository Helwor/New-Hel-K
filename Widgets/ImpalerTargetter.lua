...
local UNREGISTERED_UNIT = {name = 'unregistered unit'}
...
local function OrderDraw(str,id,color)
    ...
    if order and myDrawOrders[order] then
        order.str = str
        order.timeout = os.clock()+10
    else
        order = {str=str,type='font',pos={id},timeout = os.clock()+10,color=color}
...
local function CheckTarget(id,unit, dbgScore)
    if not unit.isEnemy and (unit.isInSight or unit.isStructure and unit.isKnown) then
        return
    end
    ...
    if unit.isStructure then
        ...
        if unit.name == 'staticmex' then
            score = score +2
        elseif ((unit.cost or 0)>=400 or unit.name=='turretriot' or unit.name == 'staticcon') then
            score = score + 1
        end
...
local function isTargetInRange(id, x,y,z)
    -- if x,y,z
end
...
function widget:GameFrame(gf)
    ...
    if impalers and next(impalers) then
        ...
        if not next(impsToTreat) then
            -- Echo('no imp to treat',math.os.clock())
            return
        end
        -- then we gather all units in range of them without duplicate
        local validTargets={}
        local dbgScore = Debug.score()
        for id,imp in pairs(impsToTreat) do

            imp.potentialTargets = {}
            local x,y,z = imp:GetPos(1)
            local inrange = spGetUnitsInCylinder(x,z,range)
            for i=1,#inrange do
                local uid = inrange[i]
                if validTargets[uid] or spValidUnitID(uid) then
        ...
        for id,unit in pairs(validTargets) do
            local score = CheckTarget(id, unit, dbgScore)
        ...
        for id,imp in pairs(impsToTreat) do
            -- NOTE: 
                -- the target set override the attack command
                -- Shooting toward stop if the targetted ground is out of range while an enemy is in range, in that case, isUserTargetBecome false and an enemy is targetted
            local tgt_type,isUserTarget,tgt_id, tgt_y, tgt_z = spGetUnitWeaponTarget(id,weapNum)
            local cmd,opt,tag,tgt_cmd,_, tgt_cmd_z = spGetUnitCurrentCommand(id)
            local shootingToward = spGetUnitRulesParam(id, "target_towards")
            if shootingToward == 0 then
                shootingToward = false
            end
            -- isUserTarget seems to be always true, 
            -- but the target may be wrong if there are attack command and another target set, the attack command will be given instead

            if type(tgt_id) == 'table' then
                tgt_id, tgt_y, tgt_z = unpack(tgt_id)
            end
            if dbgScore then
                local dbg_tgt = 'Target: '
                .. table.concat(
                    {
                        tostring(isUserTarget and 'User' or 'Auto'),
                        tostring(tgt_type == 2 and 'ground' or tgt_type == 1 and 'unit' or 'none'),
                        tgt_z and (tostring(math.modf(tgt_id)) .. ',' .. tostring(math.modf(tgt_z)))
                            or tostring(tgt_id)
                            or '--',
                        tostring(
                            'Registered: ' .. (
                                imp.ground_target and 'registered ground tgt' .. 'x'..imp.manual_target .. ',z' .. imp.ground_target 
                                or imp.manual_target and 'registered id tgt' .. imp.manual_target
                                or '--'
                            )
                        ),
                        tostring(
                            'CMD: ' .. (
                                cmd == CMD_ATTACK and 
                                    'attack '.. (
                                        tgt_cmd_z and 'ground ' .. 'x'..tgt_cmd .. ',z' .. tgt_cmd_z
                                        or tgt_cmd and 'unit ' .. tostring(tgt_cmd)
                                        or 'no param'
                                    )
                                or '--'
                            )
                ...
                OrderDraw(dbg_tgt,id,'yellow')
                
            end
            local a,b,c,d,e,f,g 
            if dbgScore then
                a,b,c,d = isUserTarget,tgt_id, imp.manual_target, imp.manual_target and imp.potentialTargets[imp.manual_target]
            end

            if imp.ground_target and not isUserTarget and not shootingToward then
                if cmd~=CMD_ATTACK or not tgt_cmd_z then
            ...
            if imp.manual_target then
                if not isUserTarget and (cmd ~= CMD_ATTACK or tgt_cmd ~= imp.manual_target) then
                    -- Echo('set false',cmd,tgt_cmd)
                    if dbgScore then
                        d = 'falsified '..tostring(d)
                    end
                    imp.manual_target = false
                    imp.ground_target = false
                else
                    -- if Debug.score() then
                    --     OrderDraw(imp.potentialTargets[imp.manual_target] and 'M' or 'm',id,'white')
                    -- end
                end
            end
            -- Echo("=> is ",'user target',isUserTarget,tgt_id,'manual target', imp.manual_target, imp.manual_target and imp.potentialTargets[imp.manual_target])
            if imp.manual_target then

                
                if not shootingToward then
                    local target_type = spGetUnitRulesParam(id, "target_type")
                    if imp.ground_target then
                        if tgt_id and tgt_id ~= tgt_cmd then
                            if cmd == CMD_ATTACK and tgt_cmd and tgt_cmd_z then
                                local x,y,z = unpack(imp.pos)
                                if z then
                                    local dist = ((tgt_cmd -x)^2 + (tgt_cmd_z - z)^2) ^0.5
                                    if dist < range then
                    ...
                    elseif imp.potentialTargets[imp.manual_target] then
                        local setToWantedTarget = false
                        -- Echo("target_type is ", target_type,cmd)
                        if target_type==TARGET_UNIT then
                            local tgtID = spGetUnitRulesParam(id, "target_id") -- NOTE: there is still the last target id even if the targetting has been cancelled ! (should fix)
                            if tgtID and tgtID == imp.manual_target then
                                setToWantedTarget = true
                                -- Echo(id,'the user has a manual target',tgtID) 
                            end
                        end
                        if not setToWantedTarget then
                            spGiveOrderToUnit(id,CMD_UNIT_CANCEL_TARGET,0,0)
                            -- Echo('targetting '..Units[target].name..', it has a score of '..score)
                            if dbgScore then
                                e = '#1 targetting '..tostring((Units[target] or UNREGISTERED_UNIT).name)..', it has a score of '..score
                            end
                            ATTACK_TABLE[1]=imp.manual_target 
                ...
                if tgt_score and tgt_score==maxScore then 
                    ...
                    if target_type==TARGET_UNIT then
                        local tgtID = spGetUnitRulesParam(id, "target_id") -- NOTE: there is still the last target id even if the targetting has been cancelled ! (should fix)
                        if tgtID and tgtID == tgt_id then
                            setToWantedTarget = true
                            -- Echo(id,'the user has a manual target',tgtID) 
                        end
                    end
                    if not setToWantedTarget then
                        ...
                        -- Echo('targetting '..Units[tgt_id].name..', it has a score of '..tgt_score)
                        if dbgScore then
                            f = '#2 targetting '..tostring(Units[tgt_id].name)..', it has a score of '..tgt_score
                        end
                        ATTACK_TABLE[1]=tgt_id
                        spGiveOrderToUnit(id,CMD_UNIT_SET_TARGET,ATTACK_TABLE,CMD.OPT_INTERNAL)
                ...
                if not tgt_score or tgt_score<maxScore then
                    ...
                    if target then
                        ...
                        -- Echo('targetting '..Units[target].name..', it has a score of '..score)
                        if dbgScore then
                            g = '#3 targetting '..tostring(Units[target].name)..', it has a score of '..score
                        end
                ...
                if dbgScore then
                    Echo("isUserTarget, tgt_id is ", a,b,  (c) and 'imp.manual_target '..tostring(c)..','..tostring(d) or '', (e or f or g or ''))
                end
            end
        end
        -- Echo("--",nextFrame,gf)
...
function widget:CommandNotify(cmd,params)
    -- Echo("cmd,params is ", cmd,params,impalersSelected)
        ...
        if cmd==CMD_UNIT_SET_TARGET or cmd == CMD_ATTACK then
            ...
            for i,id in ipairs(impalersSelected) do
                ...
                if impaler then
                    ...
                    -- Echo('MANUAL set target =>', tgtID,'cmd',cmd, 'ground target?', impaler.ground_target)
                end
            end
        end
    end
...
function widget:Initialize()
    ...
    MyUnitsByDefID=WG.UnitsIDCard.mine.byDefID
    impalers = MyUnitsByDefID[impaler_defID]
