--Use
-- -HasViewChanged, -AddSleepWake, UnitsIDCard, api_unit_handler, -OnWidgetState, command_tracker, gui_epicmenu, -VisibleUnits.lua
...
-- April 2025
	-- implemented return to original pause (TEST IT)
...
local requirements = {
	exists = {
		...
		[WIDGET_DIRNAME ..'api_cluster_detection.lua'] = {VFS.RAW},
...
for defID,def in pairs(UnitDefs) do
	if def.isFactory then
		facDefID[defID] = true
		facDefIDs[#facDefIDs + 1] = defID
	end
end
local function isCommWreck(def)
	return def.name:match('_base_') or def.name:match('dyn') -- TODO IMPROVE DEFINITION 
end
local wantedFeatureDefID = {} -- optionnable with/without comm
...
local f = f -- WG.utilFuncs
local debugMe = false
local myPlayerID = Spring.GetMyPlayerID()
local myTeamID, isSpec = false, false

local minBuildDist = math.huge
local myDrawOrders
local DrawUtils
local drawBalance = {}
local drawPos = {}
...
options_order = {
	...
	'lbl_restriction','antilag','disallow_comms','smart_conjurer','override_autoguard','allowComReclaim',
...
options = {
	...
	allowComReclaim = {
		name = 'Allow Reclaim Comm wreck',
		type = 'bool',
		value = cfg.allowComReclaim,
		OnChange = function(self)
			local wantCommWreck = self.value
			local FeatureDefs = FeatureDefs
			for defID in pairs(wantedFeatureDefID) do
				if isCommWreck(FeatureDefs[defID]) then
					wantedFeatureDefID[defID] = wantCommWreck
				end
			end
			cfg['allowComReclaim'] = wantCommWreck
		end,
	},
	-----   -----------------
	lbl_cluster = {
	...
	if Type == 'balance' then
		...
		if arg=='M' then
			...
			if orderM and myDrawOrders[orderM] then
				...
				table.insert(myDrawOrders, orderM)
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
local function IsOwnOrder(coded, cmd)
	-- Echo("SB_INTERNAL, CMD_OPT_SHIFT, CMD_OPT_ALT is ", SB_INTERNAL, CMD_OPT_SHIFT, CMD_OPT_ALT)
	return coded == SB_INTERNAL + CMD_OPT_SHIFT
		or coded == CMD_OPT_ALT + CMD_OPT_SHIFT + SB_INTERNAL
		or coded == SB_INTERNAL and cmd == CMD_FIGHT
		-- or cmd == CMD_RAW_MOVE and coded == SB_INTERNAL
end
...
local function FindIntersect(x, z, s1, s2, e1, e2) -- actually clamping a point into a segment
...
local function UpdateResources()
	...
	res.eReserve = res.eMax > 0 and spGetTeamRulesParam(myTeamID,'energyReserve') or 0
	res.mReserve = res.mMax > 0 and spGetTeamRulesParam(myTeamID,'metalReserve') or 0

	res.mMax = res.mMax - HIDDEN_STORAGE 
	res.eMax = res.eMax - HIDDEN_STORAGE
	...
	-- 	teamTotalEnergyStored = teamTotalEnergyStored + min(eCurr, eStor - HIDDEN_STORAGE)
	-- end

	res.eCur = min(res.eMax, res.eCur - res.energyIncome)
	res.eDelta = res.energyIncome - ePull - extraEnergyPull
	...
	res.mDelta = mInc - (mPull + extraMetalPull)

	--*******Echo("update",res.mDelta)
	...
	--Echo("res.eReserve is ", res.eReserve,res.eCur)
	local usable_E = ceil(res.eCur)-res.eReserve
	local usable_M = ceil(res.mCur)-res.mReserve
	...
	-- local fullbuildCap = min(ceil(res.mCur)+res.mDelta,ceil(res.eCur)+res.eDelta)
	res.buildCap = min(
		ceil(res.mCur) + res.mDelta - res.mReserve,
		ceil(res.eCur) + res.eDelta - res.eReserve
	)
	...
	-- Echo("res.Estalling, res.OverMetal is ", res.Estalling, res.OverMetal)
	if ((res.mMax - res.mCur) < mInc) then 
		res.OverMetal = true 
	else 
		res.OverMetal = false
	end
	
	-- Echo("res.mExcess is ", ceil(res.mCur), res.mDelta, res.mReserve,'||',  )
	-- Echo("res.OverMetal is ", res.OverMetal)
	...
	local eStall = res.eCur - res.eReserve + res.eDelta
	if eStall < 0 then
		res.mDelta = res.mDelta - eStall 
	end		
	res.wantM = res.mMax==0 and -res.mDelta or res.mMax - (max(res.mCur,res.mReserve) + res.mDelta)
	if res.wantM < res.mMax * 0.01 then
		res.wantM = 0
	end
	res.needM = res.mReserve - res.mCur - res.mDelta

		-- Echo('res updated ','needM: ' .. FormatNum(res.needM), 'needE: ' .. FormatNum(res.needE))
	OrderDraw('balance',Debug.showBalance() and 'M')


	res.wantE = res.eMax == 0 and -res.eDelta or res.eMax - (max(res.eCur, res.eReserve) + res.eDelta)
	if res.wantE < res.eMax*0.01 then
		res.wantE = 0
	end
	res.needE = res.eReserve - res.eCur - res.eDelta
...
function builderClass:UpdateRoam()
	...
	if unit.isFighting or self.fighting then
		if not cfg.allow_roam_on_fight then
			self.canRoam = false
			self:GetRange()
			if debug then
				Echo(self.humanName,self.id,'is not allowed to roam in fight')
			end
			return false
		end
	else
		-- Echo('checking, currentFrame', currentFrame)
		if not cfg.allow_roam then
			self.canRoam = false
			self:GetRange()
			if debug then
				Echo(self.humanName,self.id,'is not allowed to roam on place')
			end
			return false
		end
		if unit.isIdle and cfg.roam_time_out > 0 then
			-- if spGetGameSeconds() < self.idleTime + cfg.roam_time_out then
			if (currentFrame / 30) < self.idleTime + cfg.roam_time_out then
				self.canRoam = false
				self:GetRange()
				if debug then
	...
	self:GetRange()
	if debug then
		Echo(self.humanName,self.id,'is allowed to roam')
	end
	return true
end
...
function builderClass:GetFightingOrder()
	...
	if self.canRoam and (self.fighting or self.unit.isFighting) then
		local id = self.id
		local x,y,z = GetUnitPos(id, 15)
...
function builderClass:CheckSmartConjurer()
	...
	if self.fighting and self.canRoam then
	...
	if unit.isCloaked then
		if (spGetUnitRulesParam(id,'areacloaked') or 0) > 0  then
		...
		if unit.name == 'cloakcon' then
			...
			for i,cluster in ipairs(myClusters) do
				...
				if cluster.isFac then
					if diag(x-cx, z-cz) < maxAway * 2 then
						allowed = true	
					end
				elseif diag(x-cx, z-cz) < maxAway then
					allowed = true
					break
				end
				if allowed then
...
function builderClass:New(id, unitDefID, unitTeam, def)
	...
	minBuildDist = math.min(minBuildDist, def.buildDistance)
	local builder = {
		unitDefID = unitDefID,
...
local UpdateAction = function(b,cmd,params)
	...
	if cmd == CMD_RECLAIM then
		...
		--Echo("Order:",Units[bID].name,"=>",res.mDelta, "frame", frame)
	...
	if eChange ~= 0 then
		res.eDelta = res.eDelta + eChange * b.buildSpeed
		res.wantE = res.eMax==0 and -res.eDelta or res.eMax - (max(res.eCur,res.eReserve) + res.eDelta)
		if res.wantE<res.eMax*0.01 then res.wantE=0 end
		res.needE = res.eReserve - res.eCur - res.eDelta
		OrderDraw('balance',Debug.showBalance() and 'E')
	...
	if mChange ~= 0 then
		res.mDelta = res.mDelta + mChange * b.buildSpeed
		res.wantM = res.mMax==0 and -res.mDelta or res.mMax - (max(res.mCur,res.mReserve) + res.mDelta)
		if res.wantM < res.mMax*0.01 then res.wantM = 0 end
		res.needM = res.mReserve - res.mCur - res.mDelta
		OrderDraw('balance',Debug.showBalance() and 'M')
...
local function PutOnListening(b)
	...
	if Debug.prevision() and newaction then
		Echo(
			...
			,'M '.. round(mBefore,1) .. ' => ' .. round(res.mDelta,1)
			,'E ' .. round(eBefore,1) .. ' => ' .. round(res.eDelta,1)
...
function NotifyIdle(id,unit)
	...
	if b.currentAction ~= 'listening' and b.currentAction ~= 'manual' then
		local movingBack = Stop(id)
		if movingBack then
			return
		end
	end
...
function NotifyExecute(unit,id,cmd,params,opts,tag,fromCmdDone,fromLua,realcmd,realparams,realopts,realtag,maincmd,mainparams)
	...
	local isOwn = IsOwnOrder(realopts.coded, cmd)
	-- Echo("#"..id, "coded, cmd is ", realopts.coded, cmd, f.cmdNames[cmd])
	...
	if isOwn and (cmd == CMD_GUARD or cmd == CMD_FIGHT) then
		ResetUnit(id, unit, 11)
		-- Echo('our own guard => reset')
		return
	elseif not isOwn and unit.autoguard and cfg.override_autoguard then
		ResetUnit(id, unit, 1)
		return     
	end
	...
	-- Echo('res before', 'M ' .. res.mDelta,'E ' .. res.eDelta)
	...
	-- b.fighting is a pseudo fighting state when Fight order is replaced by a move order of our own
	if not isOwn then
		b.fightingOrder = false
		b.fighting = false
		b.movingBack = false
		unit.movingBack = false
		-- Echo('not own, b.fighting is now false')
	else
		if realcmd == CMD_RAW_MOVE then
			if b.movingBack then
				-- b.movingBack = false
			else
				b.fighting = true
			end
	...
	if not b.canRoam then
		-- Echo('cant roam, b.fighting is now false')
		b.fighting = false
	end
...
local ByDistFeatures = function(a,b)
	if not dist[a] then
		...
		dist[a] = diag(x-fx, z-fz)
	end
	if not dist[b] then
		local fx,_,fz = GetFeaturePosition(b)
		dist[b] = diag(x-fx, z-fz)
	end
...
local ByDistUnits = function(a,b)
	if not dist[a] then
		...
		dist[a] = diag(x-ux, z-uz)
	end
	if not dist[b] then
		local ux,_,uz = spGetUnitPosition(b)
		dist[b] = diag(x-ux, z-uz)
	end
	return dist[a]<dist[b]
end
...
local function UpdateOrder(bID,b,frame)
	...
	if cmdID == CMD_RAW_BUILD then
		isMoving = true
		cmdID,opt,tag,curID = spGetUnitCurrentCommand(bID, 2)
	end
	local currentAction = b.currentAction
	b:UpdateRoam()
	local range = b.range
	local isFighting = b.fighting and b.canMove
	-- if bID ~= 10937 then
	-- 	Echo(cmdID, "wantedAction, currentAction is ", wantedAction, currentAction)
	-- end
	b.treated = frame
	...
	if currentAction == 'repairing' then
	...
	--local minM,minE = -b.buildSpeed,-b.buildSpeed + (res.eMax>0 and res.eReserve or 0)
	...
	if res.wantM > b.buildSpeed*2 or res.wantE > b.buildSpeed*2 then
		-- Echo("res.wantM, res.wantE is ", res.wantM, res.wantE, res.needM, res.needE)
		-- Echo("res.eCur - res.eReserve + res.eDelta is ", res.eCur - res.eReserve + res.eDelta)
	...
	-- if b.isComm then
	--     Echo("Prio1, Prio2, Prio3 is ", Prio1, Prio2, Prio3)
	-- end
	...
	-- if new_target and new_target == curID and wantedAction == currentAction then
	...
	-- 	--[[if res.mCur+(res.mDelta + b.buildSpeed * mChange) < -b.buildSpeed
	-- 	or res.eCur+(res.eDelta + b.buildSpeed * eChange) < -b.buildSpeed + res.eReserve
	-- 	then
	...
	if manualTarget then
		...
		if x then 
			...
			if diag(b.posX-x,b.posZ-z) > range then
				manualTarget=false
			end
		end
	end
	...
	elseif cmdID == CMD_RECLAIM and (
			b.recM and res.mCur == res.mMax
			or b.recE and res.eCur == res.eMax
		)then
		-- res.eDelta = res.eDelta - ENERGY_BALANCE[b.recE] * b.buildSpeed
		-- res.mDelta = res.mDelta - METAL_BALANCE[b.recM] * b.buildSpeed

		Stop(bID,isFighting,tag)
	elseif curID and wantedAction == "listening" and currentAction ~= "listening" and currentAction ~= "manual" and not b.isDumb then
		-- Prio1,Prio2,Prio3,debugPrio, new_target = nil,nil,nil, nil,nil
		local x, z
		if curID > maxUnits then
			x, z = spGetFeaturePosition(curID)
		elseif Units[curID] then
...
function widget:UnitCommand(id, defID, unitTeam, newCmd, params, opts, cmdTag, playerID, fromSynced, fromLua)
	...
	if b then
		...
		if newCmd == CMD_MOVE_STATE and b.canMove then
			if unit.isComm and cfg.disallow_comms then
				return
			end
		...
		if newCmd==CMD_INSERT then
			newCmd = params[2]
			params = {select(4,params)}
		end
		if newCmd == CMD_STOP or newCmd == CMD_FIRE_STATE then
			return
		end
		...
		local current = queue[1]
		local nex = queue[2]
		-- Echo(newCmd,"trackedunit.manual is ", trackedunit.manual,'unit.manual',unit.manual,"unit == trackedunit",unit == trackedunit)
		-- Echo("trackedunit.holder is ", trackedunit.holder.creation_time,unit.holder.creation_time)
		if current then
			curCmd,curTag,curOwn,curTgt = current.id, current.tag, current.options and IsOwnOrder(current.options.coded), not current.params[3] and current.params[1]
		end
		...
		if current then
			...
			-- if not fromLua and trackedunit.manual and (curCmd==CMD_GUARD or nexCMD==CMD_GUARD) and opts.shift then
		...
		--
		-- manual command --
		-- Echo("trackedunit.manual, trackedunit.waitManual is ", trackedunit.manual, trackedunit.waitManual)
		...
		--     end
		-- end
		if not newOwn and (unit.manual or unit.waitManual --[[or newCmd and not is--]]) then
...
function widget:GameFrame(frame)
	...
	-- FIX ME need also to check for enemies and defense tower where the con wanna go
		...
		if cfg.detect_cluster or debugCluster then
			...
			if old then
				for id, unit in pairs(Units) do
					if (unit.isMine or unit.isAllied) and not (unit.isCon or unit.isComm) then
						local x, y, z = GetUnitPos(id,15)
						t[id] = {x,y,z}
					end
				end
			...
			if next(t) then
				...
				local facs = Spring.GetTeamUnitsByDefs(myTeamID, facDefIDs)
				local clusLen = #myClusters

				for i, id in ipairs(facs) do
					local unit = Units[id]
					if unit then
						clusLen = clusLen + 1
						myClusters[clusLen] = {id, n = 1, isFac = true, mid = {unit:GetPos(200)}}
					end
				end
...
local dangerousFeatures = setmetatable(
	{},
	{
		__mode = 'v',
		__index = function(self, fID)
			local obj = {}
			local allowed = true
			local weapRanges = WG.weapRanges
			local x, y, z = GetFeaturePosition(fID)
			local unitsAround = spGetUnitsInCylinder(x, z, 620) -- (620 is stinger range)   
			for i, id in ipairs(unitsAround) do
				local u = Units[id]
				if u and u.isEnemy then
					if u.defID and u.isDefense then
			...
			-- Echo("minBuildDist is ", minBuildDist, allowed)
			obj.allowed = allowed
			rawset(self, fID, obj)
			return obj
		end
...
function widget:UnitDestroyed(id, unitDefID, unitTeam)
	...
	local rec = b.rec
	if rec and featReclaimed[rec or rec-maxUnits] then
		featReclaimed[rec or rec-maxUnits]=nil
	end
	builders[id] = nil
	n_builders = n_builders-1
...
function widget:DrawScreen()
	if Debug.showBalance() and res.mDelta then
		glColor(0.5,0.7,1,1)
		glText(string.format('Metal Balance:%.0f',res.mDelta), 420,680, 25)
		glColor(0.9,0.9,0.2,1)
		glText(string.format('Energy Balance:%.0f',res.eDelta), 755,680, 25)
	end
end
...
function widget:DrawWorld()
	if cfg.detect_cluster and (cfg.temp_draw_cluster or Debug.cluster()) then
		...
		for i,cluster in ipairs(myClusters) do
			local mid = cluster.mid
			glDrawGroundCircle(mid[1],mid[2],mid[3], cfg.max_away_cluster * (cluster.isFac and 2 or 1) , 90)
		end
...
function widget:PlayerChanged(playerID)
	...
	local newIsSpec = Spring.GetSpectatingState()
	local wasSpec = isSpec and not newIsSpec
	if newIsSpec ~= isSpec then
		isSpec = newIsSpec
		if isSpec then
			for id, b in pairs(builders) do
	...
	if name == 'API Command Tracker' then
		...
		if not isSpec then
			...
			for id in pairs(builders) do
				...
				if teamID ~= myTeamID or not spValidUnitID(id) then
...
function widget:Initialize()
	...
	--     widgetHandler:RemoveWidget(self)
	--     Echo(widget:GetInfo().name..' requires CommandTracker widget')
	--     return
	-- end
	if not widget:Requires(requirements) then
		return
	end
	options.allowComReclaim:OnChange() -- this need to have the new gui_epicmenu bc it will be already preintegrated
	...
	GiveOrderToUnit = function(id,cmd,params,opt)
		...
		elseif cmd ~= CMD_PRIORITY and cmd ~= 2 then
			-- Echo('auto order given',id,cmd,params[1],f.cmdNames[cmd])
	...
	Units = WG.UnitsIDCard.units
	rezedFeatures = WG.UnitsIDCard.rezedFeatures
	-- WG.UnitsIDCard.subscribed[widget:GetInfo().name]=true
...
function widget:Shutdown()
	...
	--     Log:Delete()
	-- end
	-- if Units then WG.UnitsIDCard.subscribed[widget:GetInfo().name]=nil end
