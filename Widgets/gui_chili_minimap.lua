...
--------- Helwor Implementation
local Echo = Spring.Echo
local show_on_space, opacify_on_space = false, false
local spGetKeyState = Spring.GetKeyState
local spGetMouseState = Spring.GetMouseState
local spGetActiveCommand = Spring.GetActiveCommand
local spIsAboveMiniMap = Spring.IsAboveMiniMap
local spIsUserWriting = Spring.IsUserWriting
local suppressInteraction
local throughTrans = 0.85
local openInteractionOnLClick = true
-- local rightClickMove
local SPACE = 32


-- options.show_on_space = {
-- 	name = 'Show On Space',
-- 	type = 'bool',
-- 	value = show_on_space,
-- 	OnChange = function(self)
-- 		show_on_space = self.value
-- 	end,
-- 	path = helKPath,
--	path = minimap_path,

-- }
options.opacify_on_space = {
	name = 'Opacify On Space',
	type = 'bool',
	desc = 'Fade the minimap unless Space is pressed',
	value = opacify_on_space,
	OnChange = function(self)
		opacify_on_space = self.value
	end,
	-- path = helKPath,
	path = minimap_path,
...
options.click_through = {
	...
	-- noHotkey = true,
	items = {
		{key = 'trans', name = 'by transparency'},
		{key = 'always', name = 'always'},
		{key = 'alwaysButLClick', name = 'always but Left Click', desc = 'Allow only interaction with the MiniMap if a left click occur on it'},
		{key = 'never', name = 'never'},
	},
	-- path = helKPath,
	path = minimap_path,
	OnChange = function(self)
		throughTrans = self.value == 'trans' and options.throughTrans.value
			or self.value == 'never' and 0
			or 1.1
		openInteractionOnLClick = self.value == 'alwaysButLClick'
	end,
}
options.throughTrans = {
	name = 'ClickThrough Transparency Threshold',
	desc = 'How opaque should it be for the click to stop going through',
	type = 'number',
	value = throughTrans,
	min = 0, max = 1, step = 0.025,
	OnChange = function(self)
		options.click_through:OnChange()
	end,
	-- path = helKPath,
	path = minimap_path,
...
function widget:Update() --Note: these run-once codes is put here (instead of in Initialize) because we are waiting for epicMenu to initialize the "options" value first.
	...
	-- else
	-- end
	-- if suppressInteraction ~= tell then
	-- 	tell = suppressInteraction
	-- 	if not suppressInteraction then
	-- 		Echo('not suppressInteraction')
	-- 	end
	-- end
...
function widget:MousePress(x, y, button)
	-- if rightClickMove and button == 3 then
	-- 	Spring.SendCommands('minimap min 0')
	...
	if Spring.GetActiveCommand() == 0 then
		if --[[rightClickMove or--]] (options.leftClickOnMinimap.value ~= 'unitselection' and button == 1) or button == 2 then
			local traceType,traceValue = Spring.TraceScreenRay(x,y,false,true)
			local coord
			if traceType == "ground" then
				coord = traceValue
			end
			if --[[rightClickMove or--]] (options.leftClickOnMinimap.value == 'camera' and button == 1) or button == 2 then
...
function widget:Shutdown()
	...
	Spring.SendCommands('minimap mouseevents 1')
	Spring.SendCommands('minimap fullproxy 1')
	if hidden then
		Spring.SendCommands('minimap min 0')
	end
end
...
function widget:DrawScreen()
	...
	-- if show_on_space and not spGetKeyState(SPACE) then
	-- 	if slaved then
	-- 		gl.SlaveMiniMap(false)
	-- 		slaved = false
	-- 	end
	-- elseif not slaved then
	-- 	gl.SlaveMiniMap(true)
	-- 	slaved = true
	-- end
	...
	if opacify_on_space then
		if spGetKeyState(SPACE) and spGetActiveCommand() == 0 and not spIsUserWriting() then
			-- alpha = 0.5
			-- rightClickMove = true
			-- Echo("select(5, spGetMouseState()) is ", select(5, spGetMouseState()))
			-- if select(5, spGetMouseState()) then
			-- 	alpha = 1
			-- end
		else
			alpha = alphaMin
	...
	local currentSuppress = suppressInteraction
	suppressInteraction = not leftClickDraggingCamera and alpha <= throughTrans
	if currentSuppress ~= suppressInteraction then
		-- local intString = tostring(suppressInteraction and 0 or 1)
		-- Spring.SendCommands('minimap mouseevents ' .. intString)
		-- Spring.SendCommands('minimap fullproxy ' .. intString)
		Spring.SendCommands('minimap min 0')
		-- WG.MiniMapFaded = suppressInteraction
		updatePos = true
	elseif suppressInteraction then
		Spring.SendCommands('minimap min 0')
	end
	if --[[updatePos or--]] (lw ~= cw or lh ~= ch or lx ~= cx or ly ~= cy or last_window_x ~= window.x or last_window_y ~= window.y) then
		lx = cx
		ly = cy
		lh = ch
		lw = cw
		...
		local mx, my, mw, mh = cx*scale, (vsy-ch-cy)*scale, cw*scale, ch*scale

		gl.ConfigMiniMap(mx, my, mw, mh)
		if suppressInteraction then
			WG.MinimapPosition = {1,1,1,1}
			WG.MinimapPositionSpringSpace = {1, vsy,1,1}
			-- window:SendToBack()
		else
