...
function TaskHandler.UpdateNEW()
    -- Echo("newUpdateCount is ", newUpdateCount, hasNewInstantUpdate)
    --// type1: run it for all current tasks
    ...
    if bool then
        if traceRequests then
            if not TaskHandler.OriRequestUpdate then
                ...
                TaskHandler.RequestUpdate = function(...)
                    if math.random(30) == 1 then
                        local trace = debug.traceback()
                        if not trace then
                            return
                        end
                        if not traces[trace] then
                            ...
                            local lines = trace:explode('\n')
                                ...
                                for i = len, 1, -1 do
                                    local line = lines[i]
                                    if line:find('in function') and line:find('Widgets') then
                                        short = line
                                    end
                                end
                                if not short then
                    ...
                    return TaskHandler.OriRequestUpdate(...)
                end
            end
        end
