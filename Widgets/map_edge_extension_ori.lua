...
local function ResetWidget()
	...
	if globalDrawList then
		gl.DeleteList(globalDrawList)
		globalDrawList = nil
	end
	widget:Initialize()
end
...
function widget:Initialize()
	...
	local switch = function()
		if options.mapBorderStyle.value == 'grid' then
			options.mapBorderStyle.value = 'cutaway'
			options.mapBorderStyle:OnChange()
		elseif options.mapBorderStyle.value == 'cutaway' then
			options.mapBorderStyle.value = 'grid'
			options.mapBorderStyle:OnChange()
		end
	end
	widgetHandler.actionHandler:AddAction(widget, 'mapgrid', switch, nil, 't')
end
...
function widget:Shutdown()
	...
	widgetHandler.actionHandler:RemoveAction(widget, 'mapgrid')
	if globalDrawList then
		gl.DeleteList(globalDrawList)
		globalDrawList = nil
	end
...
function widget:DrawWorldPreUnit()
	if drawingEnabled then
		if not globalDrawList then
			globalDrawList = gl.CreateList(DrawWorldFunc)
		end
		if globalDrawList then
			gl.CallList(globalDrawList)
		else
			DrawWorldFunc()
		end
	end
