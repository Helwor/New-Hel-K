...
options["wholeTime"] = {
	...
	OnChange = function(self)
		...
		SunCourse.updateTime = math.max(SunCourse.wholeTime / 9000, 0.06)
		SunCourse.courseTime = SunCourse.wholeTime * (1 - options.nightPart.value)
	end,
	path = helk_path .. '/Course',
}
...
SunCourse = {
	...
	Increment = function(self, timeDelta, step) -- step is no more used, for now
	...
	Update = function(self, dt, force)
		if not force and (self.paused or self.stopped) then
			return
		end
		if self.currentPer == 1 then
			self.timePassed = 0
		...
		if self.timePassed >= self.nextTime then
			...
			self.nextTime = self.timePassed + self.updateTime
	...
	Initialize = function(self,t)
		...
		self.updateTime = math.max(wholeTime / 9000, 0.06)
		self.nextTime = 0
		self.timePassed = 0
		self.time = 0
		self.day = 0
...
function widget:Update(dt)
	if initialized then
		if not SunCourse.useGameTime then
			SunCourse:Update(dt)
			return
		end
	else
		...
		elseif updates == 28 then
			SunCourse:Initialize()
			-- local f = Spring.GetGameFrame()
			-- local progress = math.max(f,0) * 0.033
			local progress = Spring.GetGameSeconds() + SunCourse.startAt * SunCourse.wholeTime
			SunCourse.startAt = false
			progress = progress % SunCourse.wholeTime
			-- Echo("progress is ", Spring.GetGameSeconds(), SunCourse.wholeTime,'= '..progress)
			Echo( ('starting Sun Course at %.1f %%'):format(progress / SunCourse.wholeTime * 100) ) 
			SunCourse:Update(progress, true)
			initialized = true
