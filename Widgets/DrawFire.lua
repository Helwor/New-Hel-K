...
if true then -- maybe will make use of it later
    return false
end
...
local colors ={}
for k,v in pairs(COLORS) do colors[k]=f.upd_new(v)-1 end
...
local DefineUnit = (function()
    ...
    local function Define(id,defID)
        ...
        local isCom = ud.name:find('dyn') or ud.name:find('c%d+_base') or ud.name:find('com') and ud.name ~= 'neebcomm'
        -- Echo("isCom is ", isCom)
...
function widget:Initialize()
    ...
    if CF2 then
        -- FIXME since MouseMove has been hijacked by ClampedMouseMove we have to fix this
        ...
        if CF2.modifiedBy then
            ...
            if not next(CF2.modifiedBy,nil) then
                if CF2._GiveNonNotifyingOrder then
                    local _,num  = GetUpvaluesOf(MouseMoveFunction,'GiveNonNotifyingOrder') -- FIXME since MouseMove has been hijacked by ClampedMouseMove we have to fix this
