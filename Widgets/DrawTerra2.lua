...
local round = math.round -- math.round return a string when all widgets getting loaded for the first time, it will be updated at Initialization
...
local dir4 -- camera orientation from WG.Cam
...
--              fragment = [[
--                 uniform float overbumpMode;
--                 void main()
--                 {
--                     float p = 0.9;
--                     float p2 = 1 ;
--                     float mod = 0.0;
--                     float inAlSq = pow(gl_Color.a, p);
--                     float alpha = gl_Color.a;
--                     gl_FragData[0].rgb =  gl_Color.rgb * 1.0002;
--                     if (alpha < 1  ) {
--                         if (alpha > 0)
--                             mod = pow(1 - alpha, p) /  overbumpMode;
--                             //gl_FragData[0].a = alpha + pow(1 - alpha, p) /  overbumpMode;

--                         //else
--                             //gl_FragData[0].a = pow(gl_Color.a, 2) + (1 - gl_Color.a) / 2;
--                         //gl_FragData[0].a = gl_Color.a;
--                     }
--                     else {
--                         gl_FragData[0].a = alpha ;
--                     }
--                     gl_FragData[0].a = clamp(alpha + mod, 0, 1);
--                     mod = 1 + pow(1 - alpha, p2) / 2 ;
--                     gl_FragData[0].rgb = gl_Color.rgb * mod;

--                 }
--             ]],
--             uniformFloat = { -- specify uniform floats here
--                 overbumpMode = overbumpMode,
--                 -- myFloat4 = {0, 1, 2, 3},
--             },
--         }
--     )
--     unifLoc = gl.GetUniformLocation(shader, 'div')
-- end
...
local s_ground_on_dig_color = {0, 0.7,  0.7, 0.4}
local s_elev_color = {0, 1, 0, 1}
local s_dig_color = {0.8, 0.4,  0.3, 0.8}
...
options = {
	...
	draw_s_ground_grid = {
		...
		type            = 'bool',
		noHotkey        = drawSloppedGroundGrid,
		OnChange        = function(self)
...
do
	...
	--[[
	local checkTimes = {
		whole = CheckTime('set', 'Total'),
		bigWhile = CheckTime('set', 'Big While')
		checkTimes.points  = CheckTime('set', 'points created in big While')
	}
	for i = 1, 8 do
		checkTimes[i] = CheckTime('set', 'sub-' .. tostring(i))
	end
	--]]
	...
	CreateSloppedRectangles = function(bx, bz, bw, bh, num, float, by, needTerra)--create tables of hollow rectangles with cardinal points, clockwise(NW, NE, SE, SW, last before NW) from center to exterior, begining at  NorthWest corner, point separated by incrementation
		...
		while i < num do
			...
			--prepare next loop
			cardinals[c2], cardinals[c3], cardinals[c4], cardinals[c5], cardinals[c6], cardinals[c7] = nil
			c2, c3, c4, c5, c6, c7 = c2+2, c3+2, c4+4, c5+4, c6+6, c7+6
			cardinals[c1], cardinals[c2], cardinals[c3], cardinals[c4], cardinals[c5], cardinals[c6], cardinals[c7]
			  = 'N',           'NE',          'E',          'SE',           'S',           'SW',          'W'
			bw, bh = bw+inc, bh+inc
			--
			...
			if i > 1 then
				for p = 1, maxln do
					...
					-- if not point.out and i < lastslope+(needTerra and 3 or 0) then
		...
		--[[
		if checkTimes.whole('count', 20) then
			checkTimes.whole('reset', 'say')
			for i = 1, 3 do
				checkTimes[i]('reset', 'say')
			end
			checkTimes.bigWhile('reset', 'say')
			Echo('--')
			for i = 4, 5 do
				checkTimes[i]('reset', 'say')
			end
			checkTimes.points('reset', 'say')
			Echo('--')
			for i = 6, 7 do
				checkTimes[i]('reset', 'say')
			end
...
local function AddAlpha_new_square(layers, numborders, maxalpha)
	...
	local function AddAlphaSide(Z, amin, amax, bmin, bmax, incb, secondPass)
		...
		if Z then
			grid, othergrid = ggridZ, ggrid
		else
			grid, othergrid = ggrid, ggridZ
		end
		local point
		local ga
		local step
			...
			if ga then
				for b = bmin, bmax, incb do
					...
					if point then
						...
						if ending and point.alpha and point.alpha < 0.1 then
							grid[a][b]= nil; othergrid[b][a]= nil
						end
				...
				for b = bmin, bmax, incb do
					...
					if point then
						if point.endslope and not secondPass then
							step = numborders
						elseif not step and secondPass then
							-- if not prevstep then
							-- 	prevstep = numborders - 0.5
							-- end
							if prevstep then 
								if not point.step then
								...
								elseif prevstep > point.step then 
									point.step = (point.step + prevstep*2) / 3 -- make the rounding
									point.alpha = maxalpha - (1 - (prevstep/maxstep))^4
								end
							end
						end
						...
						if ending and (not point.step or point.alpha and point.alpha < 0.05) then
							grid[a][b] = nil; othergrid[b][a] = nil --[[ if Z then bigGrid[b][a]= nil else bigGrid[a][b]= nil end--]]
						end
						prevstep = point.step
					end
				end
...
local function ElevationFaces(gPs, by, elevPs) -- gather unhidden points of the ground rectangle along with elevation rectangle corner, and creates 2 or 3 faces used for masking
	-- FIXME NOT WORKING WELL IN EVERY CASE
	local cam = spGetCameraState()
	 -- TODO
	-- if cam.mode ~= 1 and WG.Cam then
	-- 	return ElevationFacesCamRot(gPs, by, elevPs, cam, WG.Cam.dir8)
	-- end
	local camx = cam.px
	local flipped = (cam.flipped or -1) == 1
	...
	if  lnm == 3 and not onLeft then
		if not flipped then -- will start after NW corner
			start = 2
		end
		ending = ending + 1 -- will end at NW corner in 2 cases
	end
	for i = start, ending do
		if i == ending and lnm == 3 and not onLeft then
			i = 1
		end
		local point = gPs[i]
		if point.mask then
			...
			if y < by then
				...
				if #cardinal == 2 then --(corner)
					local start, finish
					if not f then --starting first face
						start, f = true, 1 
					elseif discontinued then
						if f == 1 or discontinued == 'started' then
							finish = true --finish face: first to second corner or fourth to first
						elseif f == 2 then 
							start = true 
							discontinued = 'started' -- found new unhidden corner after finishing the first
						end
					else
					...
					if finish then
						faces[f][lnf] = {a, y, b}
						faces[f][lnf+1] = {a, elevPs[i][2], b}-- finishing the face
						if f == 2 or f == 1 and lnm == 2 then
							break -- stopping if 2 faces done or 1 face and volume is centered on screen (only one visible face)
						end
						f = f + 1
					end
					if start then
						lnf = 1
						faces[f] = {}
						faces[f][lnf] = {a, elevPs[i][2], b}
						lnf = lnf + 1
						faces[f][lnf] = {a, y, b}
					end
				else 
					if not f then -- in case the face start in the middle of a segment
						f = 1
					end
					if not faces[f] then
						faces[f] = {}
...
do
	...
	local function lines(grid, mina, maxa, minb, maxb)
		...
		local b
		local function verts(point, grida, inc)
			while point do
		...
		for a = mina, maxa, inc do
			...
			if grida then
				...
				while b <= maxb do
					point = grida[b]
					if point then
						glBeginEnd(GL_LINE_STRIP, verts, point, grida, inc)
					else
						b = b + inc
					end
				end
	...
	for i = 1, #faces do
		for p, point in ipairs(faces[i]) do
			...
			if slopeFading and color then
				...
				if point.slopeShade < minalpha then
					point.slopeShade = minalpha
				end
...
local function DrawBorders(layers, color, slope)
	...
	for i = 1, numborders do
		color[4]= alpha
		glColor(color)
		glBeginEnd(GL_LINES, DrawBorderPairs, layers[i], alpha, slope)-- <- cool stuff with GL_LINESTRIP instead of GL_LINES but I don't remember exactly how I did
...
local function DrawSlope(layers)
	...
	local heightcam = WG.Cam and WG.Cam.relDist -- or GetCameraHeight(spGetCameraState())
	-- overbump, moreoverbump, evenmoreoverbump, pushalittle, pullalittle = 
	-- 	0.99998, 	0.9999, 		0.999, 			1.0001,	     0.99999
	curPullShader = 1
	if heightcam < 30000 then
		...
		-- 	curPullShader = overbump
		-- 	Echo("overbump", overbump, pull, overbump - pull)  -- using different shader that will force display of grid lines that are eaten by little bumps of the terrain
		-- elseif heightcam > 700 then
		-- 	curPullShader = moreoverbump
		-- 	Echo("moreoverbump", moreoverbump, pull, moreoverbump - pull)
		-- else
		-- 	curPullShader = evenmoreoverbump
		-- 	Echo("evenmoreoverbump", evenmoreoverbump, pull, evenmoreoverbump - pull)
		-- end
...
function widget:DrawScreen()
	...
	if show_slopped and showCost and layers then
		local cost = layers.cost
		if cost and cost > 0 then
			local x, y = spWorldToScreenCoords(bx + bw * dir4[1], by, bz + bh * dir4[2])
			glText(layers.cost, x, y, 10,'no')
		end
	end
end
...
function WidgetSleepNotify(w, name, preloading)
	if name == myWidgetName then
		WG.DrawTerra.ready = false
		if WG.DrawTerra.working then
			Finish()
		end
		-- Echo('Sleep notified','working?', WG.DrawTerra.working)
	end
end
