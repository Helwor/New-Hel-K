...
local internalKeys = { -- TODO: Simplify the code
    'tracked',
    'forDebug',
    'isFighting',
    'isIdle',
    ...
    'building',
    'assisting',
    'cmd',     'realcmd', 'maincmd', 'actualCmd','expectedCmd',
    'params', 'realparams',                      'expectedParams',
    'waitCmd',
    'waitParams',
    'waitManual',
    'replaceCurrent',
...
local CMD_BUMPY = customCmds.BUMPY
...
local function CleanUnit(unit)
    for i, key in pairs(internalKeys) do
        unit[key] = nil
    end
end
---------- updating teamID
...
function widget:Shutdown()
    ...
    if trackedUnits then
        for id, unit in pairs(trackedUnits) do
            ...
            unit.tracked = nil
            if trackedUnits[id] == Units[id] then
                CleanUnit(unit)
            end
            trackedUnits[id] = nil
        end
