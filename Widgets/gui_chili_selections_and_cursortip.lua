...
local function GetTooltipWindow()
	...
	function externalFunctions.SetDefaultWidth()
		window.minWidth = RIGHT_WIDTH
		window:Resize(RIGHT_WIDTH - 10)
	end

	function externalFunctions.SetWidth(w)
		window.minWidth = w
		window:Resize(w - 10)
	end
	...
	function externalFunctions.SetOpacity(opacity)
		tempOpacity = false
		if window.color[4] ~= opacity then
			window.color[4] = opacity
			window:Invalidate()
		end
	end

	function externalFunctions.SetTemporaryOpacity(opacity)
		if not tempOpacity or tempOpacity ~= opacity then
			tempOpacity = opacity
			window.color[4] = opacity
			window:Invalidate()
		end
	end
	function externalFunctions.UnsetTemporaryOpacity()
		if tempOpacity then
			window.color[4] = options.tooltip_opacity.value 
			window:Invalidate()
			tempOpacity = false
		end
	end
...
local function UpdateTooltipContent(mx, my, dt, requiredOnly)
	...
	-- 	Spring.Echo('chilitooltip', chilitooltip)
	-- end
	...
	if screen0.hoveredControl then
		...
		if tooltipWindow.window.color[4] < 0.9 then
			tooltipWindow.SetTemporaryOpacity(0.9)
		end
		...
		-- Spring.Echo('above', tooltipWindow.window.color[4], os.clock())
	
		
		-- Spring.Echo('not above', tooltipWindow.window.color[4], os.clock())
		-- Unit or feature tooltip
	local thingType, thingID
	local EzTarget = WG.EzTarget
	if EzTarget then
		local v = EzTarget.v
		local s = EzTarget.s
		thingType, thingID = 'unit', v.moddedTarget or s.moddedSelect
	end
	if not thingID then
		thingType, thingID = spTraceScreenRay(mx,my, false, true)
...
function widget:Initialize()
	...
	WG.tooltipWindow = tooltipWindow
