...
local AUTO_VERT_MODE = false -- not faster and consume some memory
local MARGIN = 150
-- Config
...
local function CircleVerts2(points)
	local len = #points
...
options.autoVert = { -- not faster and consume some memory
	name = 'Auto Vertice',
	type = 'bool',
	value = AUTO_VERT_MODE,
	OnChange = function(self)
		AUTO_VERT_MODE = self.value
	end,
	hidden = true,
}
			...
			if thisEnemy ~= false then
				...
				if thisEnemy then
					...
					if not thisEnemy[1] then
						x,y,z = select(4, spGetUnitPosition(enemy, false , true))
						if x then
