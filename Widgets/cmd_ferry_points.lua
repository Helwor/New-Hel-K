...
local function nearSegment(A, B, px, pz, r)
	local ax, az, bx, bz =  A.x, A.z, B.x, B.z, x, z
...
local function nearAnyPoint(x, z, r)
	...
	for i = 1, ferryRoutes.count do
		local route = route
		if disSQ(x, z, route.start.x, route.start.z) < rsq then
			return {r = i, index = 0}
		end
		for p = 1, route.pointcount do
			if disSQ(x, z, route.points[p].x, route.points[p].z) < rsq then
				return {r = i, index = p}
			end
		end
		if disSQ(x, z, route.finish.x, route.finish.z) < rsq then
			return {r = i, index = route.pointcount+1}
		end
		local A = route.start
		local B
		local fin = route.pointcount + 1
		for p = 1, fin do
			B = p == fin and route.finish or route.points[p]
			local jap_x, jap_z = nearSegment(A, B, x, z, 10)
			if jap_x then
				route.pointcount = route.pointcount + 1
				table.insert(
					route.points,
					p,
					{
						x = jap_x,
						y = Spring.GetGroundHeight(jap_x, jap_z),
						z = jap_z,
					}
				)
				return {r = i, index = p}
