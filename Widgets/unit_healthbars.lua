...
local Cam
local visibleUnits, visibleIcons
local Units
local inSight
local addPercent
local addTitle
...
function widget:Initialize()
	...
	Units = Cam.Units
	visibleUnits = Visibles.not_iconsMap
	visibleIcons = Visibles.iconsMap
	inSight = Cam.inSight
...
local UpdateDisarmUnits = function()
	...
	for id, unit in pairs(inSight) do
		local disarmed = GetUnitRulesParam(id, "disarmed") == 1
		if disarmed then
...
do
	...
	function widget:DrawWorld()
		if not spIsGUIHidden() then
			...
			-- 	disarmUnits[id] = nil
			-- end
			if (not next(visibleUnits) and not  visibleFeatures[1]) then
				return
			end
				...
				if unit.isKnown then
					if not Spring.GetUnitRulesParam(unitID, "no_healthbar") and DrawUnitInfos(unitID, unit.defID, unit)
						...
						if not (x and y and z) then
							Spring.Log("HealthBars", "error", "missing position " .. unitID)
						else
			...
			for i = 1, #visibleFeatures do
				...
				if (valid) then
					...
					if (dist < featureDistSq) then
						...
						DrawFeatureInfos(featureInfo.id, featureInfo.defID, wx, wy, wz)
					end
				end
			end
...
do
	...
	function widget:Update(dt)
		...
		-- Test camera height before processing
		local newGameFrame = GetGameFrame()
		...
		if newGameFrame - gameFrame < 6 then
			return
		end
		UpdateDisarmUnits()
