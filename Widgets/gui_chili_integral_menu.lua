...
options = {
	...
	background_opacity = {
		name = "Opacity",
		type = "number",
		value = userBackGroundOpacity, min = 0, max = 1, step = 0.01,
		OnChange = function(self)
			background.backgroundColor = {1,1,1,self.value}
			userBackGroundOpacity = self.value
...
local function SetGridHotkeysEnabled(newEnabled)
	...
	if gridKeysEnabled then
		for i = 1, #commandPanels do
			...
			if panel.gridHotkeys and ((not panel.disableableKeys) or options.unitsHotkeys2.value) then
				panel.buttons.ApplyGridHotkeys()
			end
		end
	else
		for i = 1, #commandPanels do
			local panel = commandPanels[i]
			if panel.gridHotkeys and ((not panel.disableableKeys) or options.unitsHotkeys2.value) then
				panel.buttons.RemoveGridHotkeys()
			end
		end
	end
end
...
local function QueueClickFunc(mouse, right, alt, ctrl, meta, shift, queueCmdID, factoryUnitID, queueBlock)
	...
	local fromLast = not queueBlock
	-- Find the end of the block
	queueBlock = (queueBlock or 0) + 1
	...
	local iterationEnd = #commands + 1
	if fromLast then
		for i = iterationEnd - 1, 1, -1 do
			
			local command = commands[i]
			local cmdID = command and command.id
			if cmdID and cmdID == queueCmdID then
				queuePosition = i
				break
			end
		end
		queuePosition = queuePosition or 1
	else
...
local function ClickFunc(mouse, cmdID, isStructure, factoryUnitID, fakeFactory, isQueueButton, queueBlock)
	...
	if alt and factoryUnitID and options.altInsertBehind.value and (not fakeFactory) then
		...
		if not spGetUnitRepeat(factoryUnitID) then
			local opt = 0 + (shift and CMD.OPT_SHIFT or 0) + (ctrl and CMD.OPT_CTRL or 0)
			spGiveOrderToUnit(factoryUnitID, CMD.INSERT, {1, cmdID, opt }, CMD.OPT_ALT + CMD.OPT_CTRL)
			if WG.noises then
				WG.noises.PlayResponse(factoryUnitID, cmdID)
			end
			return true
	...
	local function UpdateProMode()
		...
		if not firstUpdate then
			if not isStateCommand then
				...
				-- else
				-- 	button:Show()				
				elseif button.x >= 10000 then
					button:SetPos(button.x - 10000)
				end
				buttonsHolder:Invalidate()
			end
		end
	end
...
local function GetButtonPanel(parent, name, rows, columns, vertical, generalButtonLayout, generalIsStructure, onClick, buttonLayoutOverride)
	...
	externalFunctions.buttonList = buttonList
	...
	function externalFunctions.GetButton(x, y, selectionIndex)
		local buttonObj = buttons[x] and buttons[x][y]
		if buttonObj then
			if not buttonObj.button.parent then
				if not selectionIndex then
					return false
				end
				buttonObj.OnVisibleGridKeyUpdate()
				parent:AddChild(buttonObj.button)
			end
			if selectionIndex then
				buttonObj.selectionIndex = selectionIndex
			end
			return buttonObj
		end
...
local function GetQueuePanel(parent, columns)
	...
	externalFunctions.buttonList = buttons.buttonList
	function externalFunctions.ClearOldButtons(selectionIndex)
...
local function GetTabPanel(parent, rows, columns)
	...
	externalFunctions.MovePro = MovePro
	...
	function externalFunctions.SetHotkeysActive(newActive)
		...
		for i = 1, #tabList do
			local tab = tabList[i]
			tab.SetHotkeyActive(hotkeysActive)
		end
	end
...
local function ProcessCommandPosition(command, unitMobilePanelSize)
	...
	for i = 1, #commandPanels do
		local panel = commandPanels[i]
		if not panel.isBuild then

			local found = panel.inclusionFunction(command.id, factoryUnitDefID, false, unitMobilePanelSize)
			if found then
				panel.buttons.AddCommandPosition(command.id)
			end
		end
	end
end
...
local function ProcessCommand(command, factoryUnitID, factoryUnitDefID, fakeFactory, selectionIndex, forceOrdersCommand, unitMobilePanelSize)
	...
	for i = 1, #commandPanels do
		...
		if found then
			panel.commandCount = panel.commandCount + 1
			
			local x, y
			if position then
				x, y = position.col, position.row
			else
				x, y = panel.buttons.CommandToPosition(command.id, panel.commandCount)
			end
...
local function ProcessAllCommands(commands, customCommands)
	...
	for i = 1, #commandPanels do
		local panel = commandPanels[i]
		panel.commandCount = 0
		if not panel.isBuild then
			panel.buttons.ResetCommandPositions()
		end
	end
	...
	if factoryUnitDefID then
		for i = 1, #commandPanels do
			...
			if panel.queue then
				if fakeFactory then
					panel.queue.ClearFactory()
				else
					panel.queue.UpdateFactory(factoryUnitID, factoryUnitID, selectionIndex)
	...
	for i = 1, #commandPanels do
		local panel = commandPanels[i]
		if panel.commandCount ~= 0 then
			tabsToShow[#tabsToShow + 1] = panel.tabButton
			panel.buttons.ClearOldButtons(selectionIndex)
			if panel.queue then
				panel.queue.ClearOldButtons(selectionIndex)
			end
			if (not tabToSelect) and panel.tabButton.name == lastTabSelected then
...
local function InitializeControls()
	...
	background = Panel:New{
		...
		noFont = true,
		padding = {0, 0, 0, 0},
		backgroundColor = {1, 1, 1, userBackGroundOpacity},
		noClickThrough = true,
		parent = mainWindow,
	}
	...
	for i = 1, #commandPanels do
		...
		local hotkey
		if panel.optionName then
			hotkey = GetActionHotkey(EPIC_NAME .. panel.optionName)
		else
			hotkey = GetActionHotkey(EPIC_NAME_UNITS)
		end

		if panel.returnOnClick then
			panel.onClick = ReturnToOrders
		end
		
		
		panel.holder = commandHolder
		panel.buttons = GetButtonPanel(commandHolder, panel.name, 3, 6,  false, panel.buttonLayoutConfig, panel.isStructure, panel.onClick, panel.buttonLayoutOverride)
		local OnTabSelect = panel.buttons.OnSelect
		...
		if panel.factoryQueue then
			...
			panel.queue = GetQueuePanel(queueHolder, 6)
			
			-- If many things need doing they must be put in a function
			-- but this works for now.
			OnTabSelect = function ()
				panel.queue.UpdateBuildProgress()
				panel.buttons.OnSelect()
			end
		end
		
		panel.tabButton = GetTabButton(tabPanel, commandHolder, panel.name, panel.humanName, hotkey, panel.loiterable, OnTabSelect)
	
		if panel.gridHotkeys and ((not panel.disableableKeys) or options.unitsHotkeys2.value) then
			panel.buttons.ApplyGridHotkeys(gridMap, (gridCustomOverrides and gridCustomOverrides[panel.name]) or {})
		end
	end
	...
	function UpdateProModeNow()
		for i, panel in ipairs({statePanel, unpack(commandPanels)}) do
			if panel.queue then
				for i, buttonObj in ipairs(panel.queue.buttonList) do
					buttonObj.UpdateProMode()
				end
			end

			for i, buttonObj in ipairs(panel.buttons.buttonList) do
				buttonObj.UpdateProMode()
			end
		end
	end	
...
local function UpdateGrid(name)
	...
	for i = 1, #commandPanels do
		local panel = commandPanels[i]
		if panel.gridHotkeys and ((not panel.disableableKeys) or options.unitsHotkeys2.value) then
			panel.buttons.ApplyGridHotkeys(gridMap, (gridCustomOverrides and gridCustomOverrides[panel.name]) or {}, true)
		end
	end
end
...
function options.tabFontSize.OnChange(self)
	if commandPanels then
		for i = 1, #commandPanels do
			local panel = commandPanels[i]
			panel.tabButton.SetFontSize(self.value)
		end
	end
end
...
function widget:Update()
	...
	if initialized and firstUpdate then
...
if WG.utilFuncs then
	WG.utilFuncs.DebugWidget(widget)
end