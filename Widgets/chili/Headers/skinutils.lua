...
function DrawWindowList(obj)
	local w = obj.width
	local h = obj.height
	local c = obj.caption

	if obj.lastw ~= w or obj.lasth~=h or obj.lastc ~= c then
		local ret
		if not obj.lastw then
			obj.texLoaded = false
			if not loaded then
				loaded = loadedTextures
			end
			if not TextureHandler.LoadTexture(0, obj.TileImage, obj) then
				-- if obj.name and obj.name:find('Attrition') then
				-- 	Echo('the tex of AttritionCounter is still not loaded',math.round(os.clock()))
				-- end
				return
			else
				-- if obj.name and obj.name:find('Attrition') then
				-- 	Echo('the tex of AttritionCounter is loaded !',math.round(os.clock()))
				-- end
			end

			local texInfo = gl.TextureInfo(obj.TileImage) or {xsize = 1, ysize = 1}
			local tw, th = texInfo.xsize, texInfo.ysize
			if tw == -1 or th == -1 then -- never happened
				Echo('WRONG TEX BY SKINUTILS BUT TEX SEEMS LOADED',obj.classname or '',obj.name or '',obj.caption or '')
				return
			else
				-- init
				obj.texW, obj.texH = tw, th
				obj.OnDispose = obj.OnDispose or {}
				table.insert(obj.OnDispose,function(self) if self.list then glDeleteList(self.list) end if self.font_list then glDeleteList(self.font_list) end end )
				-- obj.lastw = -1
				obj.texLoaded = true
				-- obj:InvalidateSelf()
				-- return -- wait one round for the request to be done in TextureHandler
			end
		end
		local redolist = obj.lastw ~= w or obj.lasth ~= h
		obj.lastw, obj.lasth, obj.lastc = w, h, c
		if redolist then
			local tw, th = obj.texW, obj.texH
			-- Echo('creating list for',obj.classname,obj.name or '',obj.caption or '','loaded texture?', (obj.TileImage or ''):sub(-15),loaded[obj.TileImage],'size of loaded',table.size(loaded),'size of tex',tw,th,'ret of TextureLoad',ret)
			if obj.list then
				glDeleteList(obj.list)
			end
			local skLeft, skTop, skRight, skBottom = unpack4(obj.tiles)
			obj.list = glCreateList(
				function()
		...
		if obj.font_list then
			gl.DeleteList(obj.font_list)
		end
		obj.font_list = gl.CreateList(
			function()
				if (obj.caption) and not obj.noFont then
					_GetControlFont(obj):Print(obj.caption, w*0.5, 9, "center", nil, obj)
				end
			end
		)
	...
	glCallList(obj.list)
	if not obj._usingRTT and obj.font_list then
		gl.CallList(obj.font_list)
	end
	gl.Color(1, 1, 1, 1)
	-- if AreInRTT() then
...
do
	IN_SCROLLPANEL, AT_X, AT_Y = false, 0, 0
	local spanels = {}
	function DrawFonts(obj, font, count, noDraw)
		offx,offy = offx or 0, offy or 0
		local scissorActive = false
		local spanel
		if obj.font_list then
			-- Echo('font drawn for',obj.classname,obj.name or '', obj.caption or '')
			if not noDraw then
				gl.CallList(obj.font_list)
			else
				-- Echo('font out', math.round(os.clock()))
			end
			-- count = (count or 0) + 1
		end
		if obj:InheritsFrom("scrollpanel") then
			if IN_SCROLLPANEL then
				-- Echo('ALREADY IN SCROLLPANEL',IN_SCROLLPANEL.name, obj.name)
			end
			spanel = {obj = obj}
			table.insert(spanels, spanel)
			IN_SCROLLPANEL = spanel
			gl.Translate(-(obj.scrollPosX or 0), -(obj.scrollPosY or 0),0)
			-- if spanel.obj.ancestor.name == 'ProConsole' then
				pop = (pop or 0) + 1
				-- Echo(obj.name,obj.classname,'push a scissor', pop,'stack',stackN,'=>', PushScissorObject(obj))
				-- scissorActive = true
				scissorActive = PushScissorObject(obj)
			-- end
			-- PushStencilMask(obj, unpack4(obj.clientArea))
		end

		-- if COUNT < 100 then
		-- 	Echo("IN_SCROLLPANEL is ", IN_SCROLLPANEL,obj.name)
		-- end
		-- local spanel = IN_SCROLLPANEL
		local ca = obj.clientArea
		local cax, cay
		if ca then
			cax, cay = ca[1], ca[2]
		else
		...
		if children then
			for i = 1, #children do
				...
				if x then
					...
					local noDraw
					-- if obj:InheritsFrom("scrollpanel") and obj.ancestor.name == 'ProConsole' then
					-- 	Echo('child',i,child.name,child.classname,'scissorActive',scissorActive,'IN_SCROLLPANEL',IN_SCROLLPANEL, IN_SCROLLPANEL.obj)
					-- end
					if spanel then
					-- 	-- local left, right, top, bot = unpack4(spanel.contentArea)
						-- if spanel.obj.ancestor.name == 'ProConsole' then
						-- 	_COUNT = (_COUNT or 0) + 1
						-- 	if _COUNT < 25 then
						-- 		-- Echo('child',i,child.name,child.parent.name,'cay',cay,'y',y,'scrollY',spanel.scrollPosY)
						-- 	end
						-- end
						-- if y < cay then
						-- 	if child.font_list then
						-- 		noDraw = true
						-- 		nod = (nod or 0) + 1
						-- 		if spanel.obj.ancestor.name == 'ProConsole' then
						-- 			-- Echo('#',nod, IN_SCROLLPANEL.ancestor.name)
						-- 		end
						-- 	end
						-- end
					end
					...
					-- gl.PushMatrix()
					gl.Translate(x,y,0)
					count = count + DrawFonts(child, child.font, count, noDraw)
		...
		if spanel then
			gl.Translate((obj.scrollPosX or 0), (obj.scrollPosY or 0),0)
			table.remove(spanels)
			IN_SCROLLPANEL = spanels[#spanels]
			if IN_SCROLLPANEL then
				gl.Translate(-(obj.scrollPosX or 0), -(obj.scrollPosY or 0),0)
			end
			if scissorActive then
				-- pop = (pop or 0) - 1
				-- Echo('pop a scissor',pop)
				PopScissor()
			end
		end
...
function DrawWindowNoList(obj)
	...
	-- end
	if TextureHandler.LoadTexture(0, obj.TileImage, obj) then
		obj.texLoaded = true
	else
		return
	end
...
-------------------------------
DrawWindow = DrawWindowList
-- DrawWindow = DrawWindowNoList
-------------------------------
...
function DrawRepeatingTiledWindow(obj)
	...
	if obj.lastw~= w or obj.lasth ~= h then
		obj.lastw, obj.lasth = w, h
		if obj.font_list then
			gl.DeleteList(obj.font_list)
		end
		obj.font_list = gl.CreateList(
...
--// =============================================================================
--//
local GenericButtonsLists

local autozero = {__index = function(t,k) rawset(t,k,0) return 0 end}
local autotbltozero = {__index = function(t,k) rawset(t,k,setmetatable({},autozero)) return t[k]  end}
local countOfSizes = setmetatable({},autotbltozero)

local buttonGroup = {}

local buttonTex = setmetatable({},autozero)
local count = 0
function DrawButtonList(obj)
	-- Echo('using LIST')
	-- count = count + 1
	-- if count > 500 then
	-- 	gl.ColorMask(false)
	-- 	Echo('switch to false')
	...
	if obj.lastw ~= w or obj.lasth~=h then
		...
		if not obj.lastw then
			...
			local twBK, thBK = texInfo.xsize, texInfo.ysize
			if twBK == -1 or thBK == -1 then
				return
			end
			local texInfo = gl.TextureInfo(obj.TileImageFG) or {xsize = 1, ysize = 1}
			local twFG, thFG = texInfo.xsize, texInfo.ysize
			if twFG == -1 or thFG == -1 then
				return
			end
			buttonTex[obj.TileImageBK] = buttonTex[obj.TileImageBK] + 1
			buttonTex[obj.TileImageFG] = buttonTex[obj.TileImageFG] + 1
			-- init
			obj.texBKW, obj.texBKH = twBK, thBK
			obj.texFGW, obj.texFGH = twFG, thFG
			obj.OnDispose = obj.OnDispose or {}
			table.insert(obj.OnDispose,function(self) glDeleteList(self.listBK) glDeleteList(self.listFG) end )
			obj.lastw = -1
		...
		local twBK, thBK, twFG, thFG = obj.texBKW, obj.texBKH, obj.texFGW, obj.texFGH
		if obj.listBK then
			glDeleteList(obj.listBK)
			glDeleteList(obj.listFG)
		end
		local skLeft, skTop, skRight, skBottom = unpack4(obj.tiles)
		obj.listBK = glCreateList(
			function()

				TextureHandler.LoadTexture(0, obj.TileImageBK, obj)
					gl.BeginEnd(GL.TRIANGLE_STRIP, _DrawTiledTexture, 0, 0, w, h, skLeft, skTop, skRight, skBottom, twBK, thBK, 0, obj.disableTiling)
				--gl.Texture(0, false)
			end
		)
		obj.listFG = glCreateList(
			function()
				TextureHandler.LoadTexture(0, obj.TileImageFG, obj)
					gl.BeginEnd(GL.TRIANGLE_STRIP, _DrawTiledTexture, 0, 0, w, h, skLeft, skTop, skRight, skBottom, twFG, thFG, 0, obj.disableTiling)
				gl.Texture(0, false)

				-- if (obj.caption) and not obj.noFont then
				-- 	local font = _GetControlFont(obj)
				-- 	font:Print(obj.caption, w*obj.alignPadding + (obj.captionHorAlign or 0), math.floor(h*0.5 - font.size*0.35) + (obj.captionAlign or 0), obj.align, "linecenter", obj)
				-- end
			end
	...
	if not done and obj.tooltip and obj.tooltip:find('Guard') then
		WG.BUTTON = obj
		done = true
	end
	gl.Color(bgcolor)
	glCallList(obj.listBK)
	gl.Color(fgcolor)
	glCallList(obj.listFG)
	if not obj.usingRTT then
		-- glCallList(obj.font_list)
	end
	gl.Color(1,1,1,1)
	if (obj.caption) and not obj.noFont then
		local font = _GetControlFont(obj)
...
-----------------------------
DrawButton = DrawButtonList
-- DrawButton = DrawButtonNoList
-----------------------------
		...
		if obj.multiline then
			if obj.parent and obj.parent:InheritsFrom("scrollpanel") then
				...
				for i = 1, len do
					...
					for i, index in pairs(todo) do
						local line = pls[index]
						font:Draw(line.text, clientX, clientY + line.y, nil, nil, obj,i,t)
					end
				end
			else
				local pls = obj.physicalLines
				...
				for i = 1, len do
					local line = pls[i]
					font:Draw(line.text, clientX, clientY + line.y, nil, nil, obj,i,len)
				end
			end

		else
			font:DrawInBox(txt, clientX, clientY, clientWidth, clientHeight, obj.align, obj.valign, obj)
		end
...
--// =============================================================================
--//
local checkboxlists = auto_table(2)
...
local function VerifyTex(tex,obj)
	if not loaded then
		loaded = loadedTextures
	end
	if not loaded[tex] then
		TextureHandler.LoadTexture(0, tex, obj)
		return
	end
	return true
end

local function Execute(tex, w, h, obj)
	local texInfo = loaded[tex].info
	local skLeft, skTop, skRight, skBottom = unpack4(obj.tiles)
	TextureHandler.LoadTexture(0, tex, obj)
	gl.BeginEnd(GL.TRIANGLE_STRIP, _DrawTiledTexture, 0, 0, w, h, skLeft, skTop, skRight, skBottom, texInfo.xsize, texInfo.ysize, 0)
end
local function GetList(tex, w, h, obj)
	local listHolder = checkboxlists[tex][w]
	if not listHolder[h] then
		count = count + 1
		local texInfo = loaded[tex].info
		local skLeft, skTop, skRight, skBottom = unpack4(obj.tiles)
		-- Echo('create new checkbox list',count, 'tex',imageBK:sub(-20),'w,h',w,h)
		listHolder[h] = gl.CreateList(
			function()
				TextureHandler.LoadTexture(0, tex, obj)
				gl.BeginEnd(GL.TRIANGLE_STRIP, _DrawTiledTexture, 0, 0, w, h, skLeft, skTop, skRight, skBottom, texInfo.xsize, texInfo.ysize, 0)
			end
...
function DrawCheckbox(obj)
	...
	local imageFG = obj.round and obj.TileImageFG_round or obj.TileImageFG
	if not (VerifyTex(imageBK, obj) and VerifyTex(imageFG, obj)) then
		obj:InvalidateSelf()
		return
	end
	...
	-- 	local parent = obj.parent
	-- 	if parent then
	-- 		Echo('NO ANCESTOR ! parent:',parent.classname or '',parent.name or '',parent.caption or '')
	-- 	end
	-- else
	-- 	Echo("obj.ancestor is ", ancestor.classname,ancestor.caption or '',ancestor.name or '','ancestor of ancestor',
	-- 		ancestor.ancestor and ancestor.ancestor == ancestor and 'verified ancestor' or 'NO ANCESTOR.ANCESTOR',
	-- 		ancestor.useRTT and 'useRTT' or '', ancestor._usingRTT and '_usingRTT' or '')
	-- end
	...
	local box = GetList(imageBK, boxSize, boxSize, obj)
	local checked = obj.checked and GetList(imageFG, boxSize, boxSize, obj)
	...
	-- if not obj.font_list then
	-- 	obj.font_list = glCreateList(
	-- 		function()
				if (obj.caption) and not obj.noFont then
					local font = _GetControlFont(obj)
					-- if not (obj.valign or ''):find('nos') then
					-- 	obj.valign = (obj.valign or '')..'nos'
					-- 	obj.caption = obj.caption .. 'A'
					-- end
					font:Print(obj.caption, tx + obj.textoffset, ty - font.size*0.35, nil, obj.valign, obj)
				end
			-- end
		-- )
	-- end
...
function DrawProgressbar(obj)
	...
	if not obj.noSkin then
		if not TextureHandler.LoadTexture(0, obj.TileImageBK, obj) then
			return
		end
		local texInfo = gl.TextureInfo(obj.TileImageBK) or {xsize = 1, ysize = 1}
		local tw, th = texInfo.xsize, texInfo.ysize
	...
	gl.Color(obj.color)
	if not TextureHandler.LoadTexture(0, obj.TileImageFG, obj) then
		return
	end
		local texInfo = gl.TextureInfo(obj.TileImageFG) or {xsize = 1, ysize = 1}
		local tw, th = texInfo.xsize, texInfo.ysize
	...
	if (obj.caption) and not obj.noFont then
		local font = _GetControlFont(obj)
		font:Print(obj.caption, x + w*0.5, y + h*0.5 - font.size*0.35 + (obj.fontOffset or 0), "center", "linecenter", obj)
	end
end
...
function DrawTabBarItem(obj)
	...
	-- 	_GetControlFont(obj):DrawInBox(obj.caption, cx, cy, cw, ch, "center", "linecenter", obj)
	-- end
	-- if obj.lastcx ~= cx or obj.lastcy ~= cy or obj.lastcw ~= cw or obj.lastch ~= ch then 
	-- 	obj.lastcx, obj.lastcy, obj.lastcw, obj.lastch = cx, cy, cw, ch
	-- 	if obj.font_list then
	-- 		gl.DeleteList(obj.font_list)
	-- 	end
	-- 	obj.font_list = gl.CreateList(
