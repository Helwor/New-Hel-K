...
Control = Object:Inherit{
	...
	----
	useRTT = false and ((gl.CreateFBO) ~= nil),
	showRedrawRTT = false,
...
function Control:Dispose(...)
	...
	if self.font_list then
		gl.DeleteList(self.font_list)
		self.font_list = nil
	end
	if self.list then
...
function Control:Invalidate()
	if not self.useRTT then
		local ancestor = self.ancestor
		if ancestor ~= self and ancestor.useRTT and ancestor._usingRTT and not (ancestor._needRedraw or self._needRedrawSelf) then -- demand to update children
		-- -- if self.classname:lower():find('button') then
		-- 	Echo('ask redraw',self.ancestor.classname, self.ancestor.name or '',self.ancestor.caption or '')
		-- -- end
			ancestor:Invalidate()
		end
	end
...
function Control:InvalidateSelf()
	if not self.useRTT then
		local ancestor = self.ancestor
		if ancestor.useRTT and ancestor._usingRTT and not (ancestor._needRedraw or self._needRedrawSelf) then -- demand to update children
		-- -- if self.classname:lower():find('button') then
		-- 	Echo('ask redraw self',self.ancestor.classname, self.ancestor.name or '',self.ancestor.caption or '')
		-- -- end
...
function Control:_CheckIfRTTisAppreciated()
	...
	if self:InheritsFrom("window") then
		if self._usingRTT then
			return (((self._redrawSelfCounter or 1) / (self._redrawCounter or 1)) < 0.2) -- if redrawing also children occurs 5 times more than redrawing only self
		else
...
local count = 0
function Control:_UpdateChildrenDList() -- used for rtt
	if not self.parent or not UnlinkSafe(self.parent) then
		return
	end
	if not self:IsInView() then
	...
	-- called to make texture for rtt
...
function Control:_UpdateAllDList()
	...
	elseif self._tex_all then
		gl.DeleteList(self.my_list or 0)
		self.my_list = nil
...
function Control:_SetupRTT(fnc, self_, drawInContentRect, ...)
	...
	-- gl.Blending (GL.ONE, GL.ONE_MINUS_SRC_ALPHA) --blending for color (for making black & pure color)
	...
	-- gl.Clear(GL.COLOR_BUFFER_BIT, 0, 0, 0, 0)
	-- gl.Clear(GL.DEPTH_BUFFER_BIT, 0, 0, 0, 0)
	...
	-- gl.DepthTest(GL.LEQUAL) --restore default mode
		...
		if Control.showRedrawRTT then
			----
			-- gl.Color(1, 0, 0, 0.5)
			----
			if not self.rttglobalcount then
				rttglobalcount = rttglobalcount + 1
				self.rttglobalcount = rttglobalcount
			end
			self.rttcount = (self.rttcount or 0) + 1
			gl.PushMatrix()
			-- gl.Scale(1,-1,1)
			
			if not drawInContentRect then
				-----
				-- gl.Rect(self.x, self.y, self.x + 15, self.y + 15)
				-----
				gl.Color(1, 0.3, 0, 1)
				local scy = -2/self.height
				local x,y = self.x-1, self.y+9
				gl.Translate(x, y, 0)
				gl.Scale(1,(scy-(y))/(y),1)
				gl.Text(self.rttglobalcount..'-'..self.rttcount,0, 0, 11, "no")
			else
				-----
				-- gl.Rect(0, 0, 15, 15)
				-----
	...
	if not font then
		-- gl.Color(1,0.5,0.3,0.8)
		-- font = WG.Chili.Font:New()
	end
	-- if drawInContentRect then
	-- 	local ca = self.clientArea
	-- 	-- gl.Translate(self.x,self.y,0)
	-- 	local contentX, contentY, contentWidth, contentHeight = unpack4(self.contentArea)
	-- 	gl.Translate(contentX,contentY,0)
	-- 	local cnt = DrawFonts(self, self.font,0)
	-- 	-- gl.Translate(-self.x,-self.y,0)
	-- 	-- gl.Translate(-ca[1],-ca[2],0)
	-- end
...
local staticRttTextureParams = {
	...
	mag_filter = GL.LINEAR, -- NEAREST cause some problem with font in multiline textbox

	-- wrap_s = GL.CLAMP_TO_EDGE,
	-- wrap_t = GL.CLAMP_TO_EDGE,
	wrap_s     = GL.CLAMP,
	wrap_t     = GL.CLAMP,
	-- format     = GL_RGBA32F_ARB, -- transparency (used with if stencil ig)
	-- format = GL.DEPTH_COMPONENT32, -- not working
	-- format = GL_RGBA16,
	-- format = GL_RGBA,
}
-----
...
function Control:CreateViewTexture(suffix_name, width, height, fnc, ...)
	...
	--
	if  (not (fbo and texColor)) or (not gl.IsValidFBO(fbo)) then
		gl.DeleteFBO(fbo)
		gl.DeleteTexture(texColor)
		---
		-- gl.DeleteRBO(texStencil)
	...
	-- gl.ActiveFBO(fbo, DrawFonts, self, self.font, 0)

	-- gl.ActiveFBO(fbo, true, DrawFonts, self, self.font, 0)
	EnterRTT()
	self._inrtt = true

	gl.ActiveFBO(fbo, true, Control._SetupRTT, self, fnc, ...) -- execute the self then children through DrawForList
	-- gl.RenderToTexture(texColor, Control._SetupRTT, self, fnc, ...) -- execute the self then children through DrawForList
	self._inrtt = false
	LeaveRTT()
	-- DrawFonts(self, self.font, 0)
...
function Control:_DrawInClientArea(fnc, ...)
	...
	local scissorActive
	if self.classname:find('scrollpanel') then
		scissorActive = PushScissorObject(self)
	end
...
local count = 0
function Control:DrawForList() -- seems to be only used when in RTT
	...
	if (self._tex_all and not self._inrtt) then
		if not self.texinit then
			local loaded
			TextureHandler.LoadTexture(0, self._tex_all, self) 
			local texInfo = gl.TextureInfo(self._tex_all) or {xsize = 1, ysize = 1}
			local tw1, tw2, th1, th2
			...
			if self._tex_children then
				TextureHandler.LoadTexture(0, self._tex_children,self)
				local texInfo = gl.TextureInfo(self._tex_children) or {xsize = 1, ysize = 1}
				local tw, th = texInfo.xsize, texInfo.ysize
				if tw == -1 or th == -1 then
					loaded = false
				end
		...
		-- Echo('here-0', count < 50 and self.classname or '', AreInRTT())
		...
		gl.PopMatrix()
		return
	elseif (self._all_dlist) then -- never called
		-- Echo('here-1', count < 50 and self.classname or '', AreInRTT())
		gl.PushMatrix()
		gl.Translate(self.x, self.y, 0)
	...
	if (self._own_dlist) then -- never the case
		-- Echo('here-2', count < 50 and self.classname or '', AreInRTT())
		...
		if self._hasCustomDrawControl then
			...
			-- Echo('here-3 draw ctrl', count < 250 and self.classname or '', AreInRTT())
			-- called while in RTT, the corresponding Draw function in skinutils mostly, and also label
			self:DrawControl()
		end
	end
	...
	if (clientWidth > 0) and (clientHeight > 0) then
		if (self._tex_children) then -- called in RTT
			-- Echo('here-4', count < 50 and self.classname or '', AreInRTT())
		...
		elseif (self._children_dlist) then
			-- Echo('here-5', count < 50 and self.classname or '', AreInRTT())
			self:_DrawInClientArea(gl.CallList, self._children_dlist)
		else
			-- Echo('here-6 draw children', count < 250 and self.classname or '', AreInRTT())
			-- called while in RTT
...
local Counter = (function()
	...
	return {
		...
		update = function()
			...
			local passed = spDiffTimers(now, timer)
			if passed > 1 then
				scx, scy = gl.GetViewSizes()
				local cnt = 0
				for k in pairs(objs) do
					cnt =  cnt + 1
					objs[k] = nil
				end
				local str = ('draw: %d/sec (%d objs)'):format(count / passed, cnt)

				gl.DeleteList(countlist)
				countlist = gl.CreateList(
					function()
						gl.Color(1, 1, 0, 1)
						gl.PushMatrix()
							gl.Translate(x, y, 0)
							gl.Scale(1,-(2/scy+y)/y,1)
							gl.Text(str,0, 0, 11, "rno")
						gl.PopMatrix()
						gl.Color(1, 1, 1, 1)
					end
				)
...
function Control:Draw() -- those are called out of RTT
	-- gl.Scale(1,-1,1)

		-- gl.Text(count,500, select(2,gl.GetViewSizes()) - 500, 11, "no")
	if Control.showDrawCount then
		Counter.update()
	...
	if (self._tex_all) then -- called out of rtt once the job is done
		if WG.ChiliRedraw then
		...
		gl.Translate(self.x, self.y, 0)
			...
			-- gl.BlendFuncSeparate(GL.ONE, GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA, GL.SRC_ALPHA)
			...
			-- count = count + 1
			-- if count < 500 then
				-- Echo("self.classname is ", self.classname)
			-- end
				...
				local scissorActive
				if self.classname:find('window') then
					scissorActive = PushScissorObject(self)
				end
				DrawFonts(self, self.font,0)
				if scissorActive then
	...
	if (self._all_dlist) then
		-- Echo('here1', count == 50 and self.classname or '', AreInRTT())
	...
	if (self._own_dlist) then
		-- Echo('here2', count == 50 and self.classname or '', AreInRTT())
		gl.CallList(self._own_dlist)
	else
		if WG.ChiliRedraw then
		...
		if self._hasCustomDrawControl then
			...
			-- called out of RTT
			-- Echo('here6', count < 150 and self.classname or '', AreInRTT())
			self:DrawControl()
		end
	end

	if (self._children_dlist) then
		-- Echo('here3', count == 50 and self.classname or '', AreInRTT())
		self:_DrawInClientArea(gl.CallList, self._children_dlist)
	else
		-- Echo('here4', count == 50 and self.classname or '', AreInRTT())
		self:DrawChildren();
	end
