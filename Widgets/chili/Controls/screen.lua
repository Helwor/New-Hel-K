...
function Screen:HitTest(x,y,...)
	-- Echo('working',self.classname,x,y,...)
	return self.inherited.HitTest(self,x,y,...)
...
function Screen:MouseDown(x, y, btn, ...)
	...
	local hovered = self.hoveredControl
	-- if hovered and btn == 1 then
	-- 	local alt, ctrl, meta, shift = Spring.GetModKeyState()
	-- 	if ctrl then
	-- 		hovered.useRTT = not hovered.useRTT
	-- 		Echo('Set RTT on ',hovered.calssname,tostring(hovered.useRTT):upper(),hovered.name,hovered.caption or '',hovered._cantUseRTT and "can't" or 'can')
	-- 		hovered:InvalidateSelf()
	-- 		return false
	-- 	elseif alt then
	-- 		local obj = hovered
	...
	-- 				ancestor.ancestor and ancestor.ancestor == ancestor and 'verified ancestor' or 'ANCESTOR~=ANCESTOR.ANCESTOR',
