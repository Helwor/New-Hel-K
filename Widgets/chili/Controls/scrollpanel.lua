...
function ScrollPanel:_DrawInClientArea(fnc, ...)
    ...
    -- if self.ancestor.name ~= 'ProConsole' then
    --     Echo("sy is ", sy,'=>', select(2, gl.GetViewSizes()) - (sy + clientHeight),'clientY',clientY,'clientHeight',clientHeight,'scale',WG.uiScale,self.classname,self.name, self.ancestor.name)
    -- end
...
function ScrollPanel:MouseWheel(x, y, up, value, ...)
    if self._vscrollbar and not self.ignoreMouseWheel then
        local s = value < 0 and -1 or 1
        value = s * (s*value)^1.7
        self:SetScrollPos(nil, math.floor(self.scrollPosY - value * 30), false)
        return self
    end
