function widget:GetInfo()
	return {
		...
		desc = "Remove usual moving orders of the manually lobbed units",
        author = "Helwor",
				...
				for _, uid in ipairs(unitsAround or EMPTY_TABLE) do
					if not done[uid] then
						...
						if spGetUnitTeam(uid) == myTeamID then
							...
							if defID and launchable[defID] and not (lobsterSelected and lobsterSelected[uid]) then
								local cmd = spGetUnitCurrentCommand(uid)
								-- if cmd and cmd ~= CMD_STOP then
								if cmd and cmd == CMD_MOVE or cmd == CMD_RAW_MOVE or cmd == CMD_FIGHT or cmd == CMD_PATROL then
...
function widget:CommandsChanged()
	lobsterSelected =  (WG.selectionDefID or spGetSelectedUnitsSorted() or EMPTY_TABLE)[lobsterDefID]
end
