function widget:GetInfo()
	return {
		...
		author    = "Lelousius and aegis, modded Licho, CarRepairer, jK, Shadowfury333, partial rewrote Helwor",
		date      = "30.07.2010",
...
local wantHoverInner = false -- color the interior of the shape for hovered unit, default
...
options_order = {'allyselectionlevel', 'showallyplayercolours', 'showhover', 'showinselectionbox', 'animatehover', 'animateselectionbox', 'wantHoverInner'}
options = {
	allyselectionlevel = {
	...
	wantHoverInner = {
		name = 'Interior shape for hovered',
		desc = '',
		type = 'bool',
		value = true,
		advanced = true,
...
local function Init()
	...
	for defID, def in pairs(UnitDefs) do
		...
		if cp.selection_scale then
			local factor = (tonumber(cp.selection_scale) or 1)
			scale = scale*factor
			xsize = xsize*factor
			zsize = zsize*factor
		end
		...
		unitConf[defID] = {
			...
			xscale = xscale,
			zscale = zscale,
			noRotate = (cp.select_no_rotate and true) or shape == shapes.circle or false,
			velocityHeading = (cp.selection_velocity_heading and true),
		}
	end
end
...
local function Draw()
	...
	if timePassed == 0 then
		...
		globalDraw = glCreateList(DrawAllShapes)
	end
