...
options.reorderGroups = {
	...
	value = opt.reorderGroups,
	OnChange = function(self)
		opt.reorderGroups = self.value
	end,
	path = helKPath,

}

options.singlePointTolerance = {
	name = "Mouse Leeway",
	desc = "How sensible it is to distinguish between line formation and single point",
	type = "number",
	min = 1, max = 30, step = 1,
	value = opt.singlePointTolerance,
	OnChange = function(self)
		opt.singlePointTolerance = self.value
	end,
	path = helKPath,
}
-- local BOMBER_SHOOT_ONCE = false
-- local WIDOW_SHOOT_ONCE = false

options.shiftAttackTrailShootToward = {
	name = "Shift + Attack Trail => Shoot Toward",
	type = "bool",
	value = opt.shiftAttackTrailShootToward,
	OnChange = function(self)
		opt.shiftAttackTrailShootToward = self.value
	end,
	path = helKPath,
}

options.altShootOnce = {
	name = "Attack trail => Shoot Once",
	type = "bool",
	value = opt.altShootOnce,
	OnChange = function(self)
		opt.altShootOnce = self.value
	end,
	path = helKPath,
}

options.singleSlowCtrl = {
	name = "Ctrl slow single selected unit",
	type = "bool",
	value = opt.singleSlowCtrl,
	OnChange = function(self)
		opt.singleSlowCtrl = self.value
	end,
	path = helKPath,

}
options.forceAlg = {
	name = 'Force Algorythm',
	desc = 'Hungarian method is used by default unless the work takes too long, this option is for testing with a fixed desired algorythm',
	type = 'radioButton',
	value = 'disabled',
	items = {
...
-- 	type = "bool",
-- 	value = BOMBER_SHOOT_ONCE,
-- 	OnChange = function(self)
-- 		BOMBER_SHOOT_ONCE = self.value
-- 	end,
-- 	path = helKPath,
-- }

-- options.widow_shootOnce = {
-- 	name = "Widow shoot once",
-- 	type = "bool",
-- 	value = WIDOW_SHOOT_ONCE,
-- 	OnChange = function(self)
-- 		WIDOW_SHOOT_ONCE = self.value
-- 	end,
-- 	path = helKPath,
-- }
...
local function CanUnitExecute(uID, cmdID, cheating)
...
local function SendSetWantedMaxSpeed(alt, ctrl, meta, shift)
	...
	if ctrl then
		...
		if not selUnits[2] and opt.singleSlowCtrl then
			wantedSpeed = wantedSpeed / 2
		end
...
local function GetFormationGroups(cmdID, units)
	...
	for i = 1, #units do
		local unitDefID = spGetUnitDefID(units[i]) or 1
...
local function TweakTarget(pos, mx, my, acquiredTarget, singleNode, alt)
	...
	if not id and not hasLobster then
		...
		--[[if targType == 'feature' then
			-- id = id + maxUnits
			-- Echo("Spring.GetFeaturePosition(id) is ", Spring.GetFeaturePosition(id))
...
local function TweakCommand(usingCmd, targType, alt, ctrl, meta, shift, forceShift, forceAlt, usingRMB, singleNode)
	...
	if (usingCmd == CMD_ATTACK) and not (ctrl or usingRMB) then
		...
		then
			-- transform the command into a set target command
			usingCmd = CMD_UNIT_SET_TARGET
			ctrl = not hasImpaler -- mod a shoot toward command
			tweaked = true
		elseif opt.altShootOnce and not singleNode and pathCandidate
			or hasPuppy 
			-- or BOMBER_SHOOT_ONCE and hasBomber and targType == 'ground' 
		then
			-- shoot once mod if user draw a path of attack points, or using bomber on ground or widow
	...
	elseif (usingCmd == CMD_UNIT_SET_TARGET or usingCmd == CMD_UNIT_SET_TARGET_CIRCLE) 
		...
		ctrl = not hasImpaler -- make the path use ctrl to have a 'shoot toward' mod
		-- meta = true
		-- forceShift = true
		tweaked = true

	elseif (usingCmd == CMD_MANUALFIRE or usingCmd == CMD_AIR_MANUALFIRE) and pathCandidate then
		ctrl = false-- spam manual fire at different location following the mouse (suppress the shift)
		meta = false
		forceShift = false
		tweaked = true
...
local function SetOrder(targType, forceShift, forceAlt, usingRMB, pos, tweakTarget, mx, my, acquiredTarget, singleNode)
	local  alt, ctrl, meta, shift = GetModKeys()
	local tweaked
...
function widget:MousePress(mx, my, mButton, byEz)
	...
	if activeCmdID then
		...
		if not defaultCmdID then
			...
			ttype, tid = targType, targID
			if targType then
				-- Echo("ttype, tid is ", ttype, tid)
...
function widget:MouseRelease(mx, my, mButton)
	...
	elseif usingFormation then
		...
		do
			...
			local tweaked
			tweaked, usingCmd,  alt, ctrl, meta, shift, usingContextCommand = TweakCommand(usingCmd, targType, alt, ctrl, meta, shift, forceShift, forceAlt, usingContextCommand, singleNode)
			if tweaked then
...
-- Command Handling
--------------------------------------------------------------------------------
local rankCandidateDefID = {}
for defID, def in pairs(UnitDefs) do
	if def.canMove and not def.isFactory and not def.springCategories.fixedwing then
