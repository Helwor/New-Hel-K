...
-- April 2025 use now .linkOption instead of dirty workaround
...
local function GetFovFactor(fov)
    local currentFOVhalfRad = (fov/2) * RADperDEGREE
    local fovFactor = tan(currentFOVhalfRad) * (45 / fov)
    return fovFactor
end
...
options.fov = {
        ...
        simpleMode = true,
        everyMode = true,
        linkOption = {'Settings/Camera/COFC Controls', COFCName .. 'fov'},
        tooltipFunction = function(self)
            -- Echo('Tooltip FOV')
            local cs = spGetCameraState()
...
options.maxzoomout = {
        name = 'Max Zoom Out',
        -- must be updated/update into the code, must adapt to FOV change
        -- desc = '% of the average h/w map fitting screen',
...
options.maxzoomout_alt = {
        name = 'Max Zoom Out Alt',
        -- must be updated/update into the code, must adapt to FOV change
        desc = '% of the average h/w map fitting screen if Alt is held',
        type = 'number',
        min = 0.5, max = 3, step = 0.01,
        value = 1,
        tooltipFunction = function(self)
...
function WidgetRemoveNotify(w,name,preloading)
    ...
    --     UnlinkOptions(COFC.options.maxzoomout,options.maxzoomout_alt)
    --     COFC = nil
    --     origCOFCFovOnChange = nil
    --     options.fov:OnChange()
    --     options.maxzoomout.hidden = true
    --     options.maxzoomout_alt.hidden = true
...
function widget:Initialize()
    ...
    --     LinkOptions(COFC.options.maxzoomout_alt,options.maxzoomout_alt)
...
function widget:Shutdown()
    ...
    --         UnlinkOptions(COFC.options.maxzoomout,options.maxzoomout)
    --         UnlinkOptions(COFC.options.maxzoomout_alt,options.maxzoomout_alt)
    --     end
    -- end
    -- if WG.MyClicks then
