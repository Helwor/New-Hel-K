...
--                                                                  | only changed if catching up is detected
    {'Shadows',               0,    'Shadows'                                           },
    {'Projectile Lights',                                                               },
    -- {'Units on Fire',                                                                },
    {'HighlightUnit GL4',                                                               },
    ...
    -- very crappy appearance // since last engine it is read only,
    -- {'distdraw',         100,    'UnitLodDist'                                       },

    {'advmodelshading',       0,    'AdvUnitShading'                                    },

     -- this improve performance a lot but some map become very ugly
    -- {'advmapshading',       0,   'AdvMapShading'                                     },

    -- very crappy appearance // since last engine it is read only
    --{'distdraw',              0,    'UnitLodDist'       ,100                          },
...
function LEVELS:Down() -- going down and applying setting of that lower level (going down is going further in the array)
    ...
    if not configName then -- param is a widget
        ...
        if spGetConfigInt(configName) > value then -- in case the user has changed the config meanwhile
...
function LEVELS:Up() -- applying back value of the current level then going up (going up is going toward the start of the array)
    ...
    if not configName then -- param is a widget
        ...
        if spGetConfigInt(configName) < back_value then -- in case the user has changed the config meanwhile
...
---*** TEST 1***
    ...
    if levelChanged then -- // now using widget Sleep instead of shutdown/restart, much smoother// since the last engine version we experience some noticeable freeze on level change that can fuck up the result so we ignore this cycle
        if debugging then
    ...
    local cs = spGetCameraState()
    local speed = speedFactor * (WG.Cam and WG.Cam.relDist or GetCameraHeight(cs))/2000 * (1+dt*2)
...
function widget:GameFrame(gf)
    ...
    if gf>=target_frame then
        local cs = spGetCameraState()
        local flip = -(cs.flipped or -1)
        local speed = speedFactor * (WG.Cam and WG.Cam.relDist or GetCameraHeight(cs))/2000 * 0.50
