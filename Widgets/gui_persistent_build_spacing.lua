...
function widget:Update(dt)
    ...
    if not cmdID then
        draw = false
        buildStarted = nil
        if lastCmdID then -- fix an eventual spacing change at the last moment before ending active command
            buildSpacing[lastCmdID] = spGetBuildSpacing()
            lastCmdID = false
        end
        return
    elseif cmdID ~= lastCmdID then
