...
local spGetFeatureDrawFlag      = Spring.GetFeatureDrawFlag
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
...
local function HilightModel(drawFunc, drawData, outline)
  ...
  glTexGen(GL_T, GL_EYE_PLANE, 0, (1 / scale * 1.5), 0, shift)
  glTexture(texName)
...
local function SetFeatureColor(featureID, alpha)
  if spGetDefaultCommand() ~= CMD_RECLAIM and select(2,spGetActiveCommand()) ~= CMD_RECLAIM then
    glColor(0.7, 0.2, 0.7, 0.1)
    return
  end
...
local function HilightUnit(unitID)
  ...
  SetUnitColor(unitID, outline and ALPHA or ALPHA/2)
  HilightModel(UnitDrawFunc, unitID, outline)
end


local function HilightFeatureModel(featureID)
  SetFeatureColor(featureID, ALPHA)
  HilightModel(FeatureDrawFunc, featureID, true)
end


local function HilightFeature(featureID)
  -- Echo(featureID, "=> is ", Spring.GetFeatureDrawFlag(featureID))
  if spGetFeatureDrawFlag(featureID) ~= 17 then -- 0 no vis, 34 faded, 17 vis
    return
  end
...
function widget:Update()
  ...
  local uid =  WG.PreSelection_GetUnitUnderCursor()
  if uid then
    type, data = 'unit', uid
  end
  if type == 'unit' and spIsUnitICon(data) then
    type = ''
  end
end
