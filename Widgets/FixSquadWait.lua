...
function widget:CommandNotify(cmdID, params, opts)
	...
	if cmdID == CMD_GATHERWAIT then  -- as the gather wait is instant, we transform this order into the squad wait active command
		local selDefIDs = WG.selectionDefID or spGetSelectedUnitsSorted()
		for defID in pairs(selDefIDs) do
			if factoryDefID[defID] then
				spSetActiveCommand('SquadWait')
				return true
			end
		end
	elseif cmdID == CMD_SQUADWAIT then -- add a shift to the squad wait command if there's not, bc it is useless without it
		if not opts.shift then
			opts.shift = true
			spGiveOrder(cmdID, params, opts)
			return true
		end
	end
end
