...
local function SelectUnits(arr, shift)
	if g.SelFiltering.value == true then
		-- can put it back on the next update round
		g.switchBackSelFiltering = true
		g.SelFiltering.value = false
		g.SelFiltering:OnChange()
		-- Echo('set filtering off')
	end
	-- spSelectUnitArray(arr, shift)
	local n = arr.n -- the key 'n' is taken as an index of value by Spring.GetSelectedUnits and might provoke unwanted selection when passing by widgetHandler.UpdateSelection then widgetHandler.SelectionChanged if some widget intervene
	arr.n = nil
	spSelectUnitArray(arr, shift)
	arr.n = n
end
local function SelectUnitMap(map, shift)
	if g.SelFiltering.value == true then
		g.switchBackSelFiltering = true
		g.SelFiltering.value = false
		g.SelFiltering:OnChange()
	end
	return spSelectUnitMap(map, shift)
end
-- local g =  setmetatable(
-- 	{},
-- 	{
-- 		__newindex = function(self, k, v)
-- 			if k == 'gotNewCall' and v == true and call and call.name:match('with') then
-- 				Echo('..........\n' .. debug.traceback())
-- 			end
-- 			g[k] = v
-- 		end,
...
local partialNoSelect = 0
local useScreenCircle = true
...
local NO_SELECT_MULT = {
	[ UnitDefNames['cloakraid'].id ] = 0.5,
	[ UnitDefNames['shieldraid'].id ] = 0.5,
	[ UnitDefNames['vehraid'].id ] = 0.5,
	[ UnitDefNames['cloakassault'].id ] = 1.5,

}
local NO_SELECT_FUNC = function(id, unit, now, threshold)
	return	now + (NO_SELECT_MULT[unit.defID] or 1) * (threshold or NO_SELECT_TIMEOUT_DEFAULT)
end
...
local hotkeysCombos = {
...

	{
		...
		-- defs = {'isUnit'},
		-- on_press = true,
		longPressTime = 0.08, -- longPress is evaluated true within that time
		color = {0.2, 0.5, 1, 0.99},
		fading = 0.5,
...
options.no_select_on_partial = {
	...
	OnChange = function(self)
		partialNoSelect = self.value
	end,
}
options.use_screen_circle = {
	name = 'Use Screen Circle Selection',
	desc = 'Use flat screen circle for catching units instead of ground cylinder method',
	type = 'bool',
...
g.onClick = false
g.clickTime = osclock()
local longPress = {key = false, active = false, time = osclock(), mouseStill = false, mx =-100, my =-100}
...
g.screenradius = g.radius / SCREEN_RATIO
local memRadius = {} -- radius for each macros memorized over game
g.lagFactor = 0


local function CalcRadius()
	local camHeight = (Cam and Cam.relDist or GetCameraHeight(spGetCameraState()))
	local ratio = camHeight / BASE_CAMERA_HEIGHT
	local screen_ratio = ratio > 0.3 and ratio or ratio + (0.3 - ratio) / 2
	g.radius =(call.radius or DEFAULT_RADIUS) * (call.fixed_radius and 1 or (0.8 + (ratio-1)/zoom_scaling)) -- * (1 - (1 - (GetCameraHeight(spGetCameraState()) / 2000) ) /zoom_scaling)
	g.screenradius = g.radius / (SCREEN_RATIO * screen_ratio)
end
-----------
--- drawing variables
local x, y, z
...
local function CheckClick()--detect missed clicks and release from Update round, also care about long click
	...
	-- if g.onClick and osclock()-g.clickTime>0.66 then
...
local function MergeSelToPrev(prev, sel) -- this is not used and might be broken/ not correctly implemented
	local byID = prev.byID
	local Type = call.byType
	local byType
	local dbgPrev = Debug.previous()
...
local 	function UpdateChain(time)
	...
	if call.calledFrom or deltatime < 0.5 then
		last.chainCount = last.chainCount + 1
		table.insert(last.chained, {
		...
		if last.chainCount > maxcount and deltatime < 0.5 then
...
local function TerminateCall()
	...
	call.duration = osclock() - call.clock

	local thiscall = last.chained[#last.chained]
	if not thiscall or thiscall.call ~= call then
		Echo('EzSelector: ' .. 'ERROR, the current call about to get terminated differ from the last chained call !', thiscall and thiscall.name, call.name)
	end
...
local function RealizeCall(call, selecting, acquired, lastSel, success)
		...
		for i, id in ipairs(acquired) do
			...
			if unit then
				...
				for k, v in pairs(call.mark_unit) do
					marked[k] = type(v) == 'function' and v(id, unit, now) or v
				end
				unit.marked = marked
			end
		end
	...
	if selecting  then
		...
		local final, f = acquired, acquired.n
		...
		if namecom and namecom == 'Fight' then
			...
			-- g.verify = f + (shift and #currentSel or 0) -- work around bug, it can happen a unit having the ID of the length of the selectionh may be selected too
		-- end
	...
	if call.set_active_command and (acquired[1] or call.method == 'set_command') then 
		...
		elseif call.set_active_command <= 0 then
			...
			for i, id in ipairs(acquired) do
				comID = spFindUnitCmdDesc(id, call.set_active_command)
				if comID then
					break
				end
			end
...
function widget:CommandNotify(cmd)
	...
	then
		...
		for i, id in ipairs(currentSel) do
			local unit = Units[id]
			if unit then
				unit.marked = {no_select = NO_SELECT_FUNC(id, unit, now, partialNoSelect)}
			end
		end
	end
end
do
	local thisround = 0
	function widget:UnitCommandNotify(id, cmd)
		-- Echo("selectionResized is ", fullSel and fullSel.partial < 1)
		local now = osclock()
		if now - thisround < 0.01 then
			return
		end
		thisround = now
		if cmd == CMD_RAW_MOVE and partialNoSelect > 0 --[[and currentSel[1]--]] 
...
local function FinishCall(selecting)
	...
	-- Echo('name', call.name, 'keep', keep_on_fail, 'selecting', selecting)
	call.time_length = osclock() - call.clock
	call.finished = 'finished'
	-- Echo('finish call', call.name, #g.acquired)
	local byID = acquired.byID
	...
	if (success and call.try_on_success or retry) and not call.isTried then
		...
		call.calling = trycall
		trycall.calledFrom = call
		TerminateCall()
		g.hkCombo = trycall
		g.gotNewCall = true
		Process(0)
		...
		if takeover then
			...
			-- Echo('back to my call')
			trycall.calling = mycall
			mycall.calledFrom = trycall
			if call ~= mycall then
				TerminateCall()
				call = mycall
			end
		...
		-- Echo('trying ...', osclock(), trycall and trycall.name, 'takeover?', takeover)


	end
	
	local ignore_on_fail = call.ignore_on_fail or false
	...
	local unique_second_call = call.unique_second_call
	...
	local call_on_empty_selection = not currentSel[1] and call.call_on_empty_selection
	...
	if jumpCall then
		...
		jumpCall.calledFrom = last.call
		g.gotNewCall = true
		-- Echo(last.call.name, 'ON '..(call_on_empty_selection and 'EMPTY SEL' or call_on_fail and 'FAIL' or call_on_success and 'SUCCESS') .. ' => ' .. g.hkCombo.name)
		Process(0)
	...
	if second_call then
		...
		-- Echo(last.call.name, 'SECOND CALL =>', g.hkCombo.name) 
		g.hkCombo.secondary = last.call.secondary or last.call
		g.gotNewCall = true
		Process(0)
	end
...
do ---- **INITIALIZATION** ------
	...
	function HKCombos:Add()
		...
		for i, macro in ipairs(self) do
			...
			if macro.longPressTime and not macro.isShortPressVersion then
				...
				-- if macro.name == 'Alt Assault' then
				-- 	Echo("shortPress_version is ", shortPress_version and shortPress_version.name)
				-- end
				if not shortPress_version then
					...
					shortPress_version.shortTimed = true
				end
				...
				if next(orDouble) then
					...
					if shortPress_version2 and shortPress_version2 ~= shortPress_version then
						...
						shortPress_version2.isShortPressVersion = true
					end
				end
			...
			if macro.doubleTap_time and not (macro.isSingleTapVersion) then
				local keysByKey = {}
				-- Echo('--')
				local hasSpam
				for i, key in ipairs(macro.keys) do 
				...
				singleTap_version.isSingleTapVersion = true
				macro.doubleTap_time = nil
			end
			if macro.fastDoubleTap_time and not macro.isNon_fastDoubleTap_version then
				local keysByKey = {}
				local hasDoubleTap
				...
				non_fastDoubleTap_version.isNon_fastDoubleTap_version = true
	...
	OptimizeDefs = function(defs, combo)
		...
		for k, v in pairs(defs) do
			if type(v) == 'table' then
				...
				local strippedK = type == 'string' and k:gsub('[?!]', '')
				local DBG
	...
	function HKCombos:Set(set, i) -- set proper parameters for each combo on initialization
		...
		combo.high_lock_call = set.high_lock_call -- prevent a more complex call to be triggered
		...
		combo.hasStructure = set.hasStructure -- fasten the filtering by removing from myUnits (base) any structure building if this prop is not present which is the case most of the time
		combo.anyBP = set.anyBP -- can be selected under the 80% bp default threshold
		--
		...
		if set.groups then -- select the first matching definition found near the cursor to be the used one during that call
		...
		if combo.switch or set.shared_switch or combo.groups then
			combo.switch_on_identical = set.switch_on_identical -- switch to another definition when found units are the one already selected
		end
		combo.add = OptimizeDefs(set.add, combo) -- if at least one unit got found during process, adding units matching this def to the future selection
		...
		if set.same_units then
			-- similar units as previous selection (plate and factory are considered the same)
		...
		-- same transporting/transported state as first found during process
		-- modify temporarily call.defs and call.switch defs to match those states
		combo.same_transported_state = set.same_transported_state
		combo.same_transporting_state = set.same_transporting_state
		
		--
		combo.fail_on_identical = set.fail_on_identical -- consider the call failed if it acquires same units as the current selection
		...
		combo.force_finish = set.force_finish -- a call forced to be finished (accomplished) instead of terminated

		combo.ifSecondary = set.ifSecondary -- not fully implemented/tested, apply those property if the call if secondarily launched by another
		...
		combo.OnSuccessFunc = set.OnSuccessFunc -- run that func if the call was successful


		combo.longPressTime = set.longPressTime -- user defined time threshold for a press to be considered long ('longPress' or 'longClick' in keys)
		combo.doubleTap_time = set.doubleTap_time -- user defined time threshold for tap or click to be considered double ('doubleTap' or 'doubleLClick' or 'doubleRClick' (...) in keys)
		combo.fastDoubleTap_time = set.fastDoubleTap_time -- user defined time threshold for double tap or double click to be considered a fast double tap or click ('fastDoubleTap' or 'fastDoubleLClick' (...) in keys)


		combo.disable_SM = combo.disable_SM or set.disable_SM
		---------
		combo.option_name = set.option_name -- for hotkey modifying option -- not used anymore
		...
		if set.prefer then -- setup refined preferences that will override the matched but also added into 'previous' system
			...
			if set.avoid_non_pref then
				-- don't pick up non prefered if we got some prefered in previous system
				-- TODO VERIF IF IT DOESN'T NEED EMPTY_TABLE TOO
				combo.avoid_non_pref = true
			else
				-- Add an empty table at the end to represent units not fitting any preference
				-- treat the non prefered as prefered but with the lesser value
				table.insert(set.prefer, EMPTY_TABLE)
			end
		...
		-- ways of selecting
		combo.continue = set.continue -- resetting call continuesly (unused, not recommended to use for performance)
		...
		combo.ignore_on_fail = set.ignore_on_fail
		combo.clock = osclock()
		...
		--
		if set.share_radius then -- use the same radius of that named combo
		...
		if set.no_select then -- don't select anything
			combo.no_select = true
		end
		if set.try_on_success then -- try another macro if that one succeed
		...
		if set.cycle then 
			...
			if set.time then
				-- reset to first item after a given time
				combo.time = set.time
				combo.timecheck = set.time and CheckTime("start") or nil
			end
		end
		--
		combo.doRepeat = set.doRepeat -- repeat call on longPress (unused, not recommended for performance)
		combo.force = set.force -- override regular hotkeybound (but not the building ones for now)
		combo.selname = set.selname -- use selected unit name to appeear instead of the combo name


		-- radius for cylinder method, memorized for each combo over games
		if set.method == 'cylinder' then
			combo.radius = set.radius or memRadius[set.name] or DEFAULT_RADIUS
		end
		...
		if usePreviousSystem then
			...
			combo.previous = {byID = {}, n = 0, time = osclock(), previous_time = combo.previous_time}
		...
		for _, param in pairs({'call_on_fail', 'call_on_success', 'call_on_empty_selection', 'call_on_only_defs', 'second_call'}) do
			local subject = set[param]
			if subject then
				if param == 'call_on_only_defs' then
					for defs, callName in pairs(subject) do
						-- combo key got already updated when optimizing defs, more above
						-- we only update the corresponding chained call
						local chainedCall = self.byName[callName]
						if chainedCall then
							combo.call_on_only_defs[defs] = chainedCall
						end
					end


				else
					local chainedCall = self.byName[subject]
					if chainedCall then
						combo[param] = chainedCall
						if combo.on_press then
							chainedCall.shortTimed = true
						end
					end
					if param == 'second_call' then
						combo.unique_second_call = set.unique_second_call
					end
				end
			end
		end
		...
		combo.duration = 0
		combo.no_force = set.no_force -- don't block the key if the call is triggered
		combo.need_zero_command = set.need_zero_command -- ignore the call if a command is active
		---
		if not (combo.defs or combo.switch) then
			combo.defs = {}
		end

		combo.shortTimed = combo.on_press
	...
	function widget:Initialize()
	    ...
	    if not WG.Chili then
	    	Echo(self:GetInfo().name .. ' will not run, Chili is required')
	    	widgetHandler:RemoveWidget(self)
	    	return
	    end
...
do
	...
	local function FindTabHotkeys() -- annex of HotkeyIsBound
		if not WG.IntegralVisible and not IM then
			return EMPTY_TABLE
		else
			local cached = next(cache)
			if cached then
				return cached
			end
		end
	...
	function AfterMousePress(mx, my, button, from, locked, owner)
		...
		if not currentCombo.raw[buttonName] then
			-- if not widgetHandler.mouseOwner or widgetHandler.mouseOwner:GetInfo().name~= 'Pan View' then
				...
				-- if owner~= 'EzTarget' and owner ~= 'Pan View' and owner ~= 'Draw Placement' 
				-- 	and owner ~= 'CustomFormations2' and owner~= 'Mex Placement Handler'
				-- then
	...
	function widget:KeyPress(key, mods, isRepeat, keyset)
		...
		if not longPress.active then -- updating current Combo and verify if there's a matching call
			...
			if cckeys[symbol] then
				Echo('the key ', symbol, key, 'has already been registered !', osclock() - g.lastPress)
			end
			cckeys['mouseStill'] = nil
			cckeys['longPress'] = nil
			cckeys['longClick'] = nil
			cckeys[symbol] = l(cckeys)+1
			-- Echo('pressed ' .. symbol .. ' at ' .. l(cckeys))
...
--[[			local tmpToggle = locks.tmpToggleByKey[symbol]
			if tmpToggle then
				...
				Echo("HKCombos:Find(cckeys) is ", HKCombos:Find(cckeys))
			end--]]
			---
			local newPressTime = osclock()
			...
			local doubled, fast_doubled = nil, nil
			if last.previousKey == key then
				local time_double = newPressTime-g.pressTime
				if time_double then
				...
				-- in case there's no call on that key we need to reset the threshold preventively
				-- TODO: arrange thresholds only once and neatly
			...
			cckeys[doubleKey] = doubled
			cckeys['fast' .. doubleKey:gsub('double', 'Double')] = fast_doubled
			cckeys['spam'] = spam or nil
			...
			--Echo(osclock()-g.pressTime)
			--------------------------
			local newcall = not WG.panning and HKCombos:Find(cckeys)
			if newcall then
				if newcall.need_zero_command and spGetActiveCommand() ~= 0
					-- call require to not have any active command
				or call and (
					newcall == call 
					or newcall == call.calledFrom
					or newcall == call.secondary
					or call.high_locked
				)
					-- or prevent infinite loop
				then
					g.gotNewCall = false
					return isCtrlGroup
				end
			...
			-- found a call?
			if newcall then 
				-- following properties of that call has been added through the registration process to prepare threshold of double tap and such of more complex call that may come just after
				...
				g.hkCombo = newcall
				-- Echo("newcall is ", newcall and newcall.name, osclock())

				if newcall.removekey then cckeys[symbol] = nil end
		...
		elseif not (call and (call.locked or call.high_locked)) then---- checking if any combo has longPress if we're allowed to
			...
			cckeys['long'..(click and 'Click' or 'Press')] = 0
			cckeys['long'..(click and 'Press' or 'Click')] = nil
			local callCheck = HKCombos:Find(cckeys)
			if callCheck then
		...
		if g.hkCombo then --
			...
			-- local exec = Process(GetLastUpdateSeconds())
			local exec = Process(0)
			
			--[[ call.ifSecondary.no_force can work but is not used
			if exec and not no_force then
				if call then
					if call.secondary and call.ifSecondary and call.ifSecondary.no_force then
						no_force = true
					end
				end
			end
...
function widget:SelectionChanged(newsel)
	...
	if selectionResized then
		...
		for i = 1, n do
			...
			if not spValidUnitID(id) or spGetUnitIsDead(id) then
				...
				Echo('found invalid in SelectionChanged', id, spGetUnitIsDead(id), Units[id])
				table.remove(newsel, i)
				off = off - 1
				-- if selectionResized[id] then -- comment to avoid a double check?
	...
	if not selectionResized and newsel[1] then
		...
		-- 	-- work around bug, it can happen a unit having the ID of the length of the selection may be selected too
		-- 	for i, id in ipairs(newsel) do
		-- 		if id == n then
		-- 			table.remove(newsel, i)
		-- 			return newsel
	...
	if g.switchBackSelFiltering then
		-- put back rank filtering, gui_selection_hierarchy's SelectionChanged has already passed
		if g.SelFiltering.value == false then
...
-- 	if cmd ~= 0 then
-- 		if changeCommand then
-- 			Echo('engineCmd', cmd, changeCommand, 'time', math.round(osclock()))
...
function widget:UnitDestroyed(id, defID, teamID)
	...
	local valid = g.validUnits
	if valid.byID[id] then
		valid.byID[id] = nil
		valid.n = valid.n-1
	end
	local acquired = g.acquired
	if acquired.byID[id] then
		acquired.byID[id] = nil
		acquired.n = acquired.n-1
	end
		...
		if hadNone then
			if call.call_on_only_defs then
				...
				for defs, thiscall in pairs(call.call_on_only_defs) do
					...
					if n == acquired.n then
						thiscall.calledFrom = call
						call.calling = thisCall
						call.success = true
						TerminateCall()
						g.gotNewCall = true
						g.hkCombo = thiscall
...
do
	...
	CheckRequirements = function(reqTable, id, Or, Inv, cnt, fromTable)
		if not spValidUnitID(id) or spGetUnitIsDead(id) then
			local tickSound = LUAUI_DIRNAME .. 'Sounds/buildbar_rem.wav'
			Spring.PlaySoundFile(tickSound, 0.95, 'ui')
			Echo('found invalid in CheckRequirement', id, spGetUnitIsDead(id), Units[id])
			table.remove(newsel, i)
			return false
		end
		...
		for reqProp, reqVal in pairs(reqTable) do
			...
			--CASE: CONDITION IS A PROPERTY WITH MATCHING VALUE(s)
				...
				elseif reqProp == 'hasOrder' then
					...
					local queue = mem[id]
					if not queue then
						queue = spGetCommandQueue(id, 4) or EMPTY_TABLE
						mem[id] = queue
					end
					...
					for i, order in ipairs(queue) do
						...
						if reqVal == 'building' or reqVals and reqVals['building'] then
							...
							if val<0 then
								val = 'building'
								queue.building = val
							elseif ( val == CMD.REPAIR and not params[2]) then
					...
					if reqVal._opti then -- for table of names and the like, we previously transformed indexed table into paired table
						...
						for _, subReqVal in ipairs(reqVal) do
							...
							if reqProp == 'reload' then -- in that case, we couldnt define the prop in advance, note: even for a single weapon num to check, it has to be in a subtable
								...
								pass = reloadState and reloadState<spGetGameFrame()
							else
								pass = val == subReqVal
							end
							if subInv then pass = not pass end
	...
	function CheckAND(defsAND, allValid, pool, comment, multiCheck, debugMe) 
		-- check AND defs, find different units matching different defs
		local current_n = 0
		local found = true
		for i, def in ipairs(defsAND) do
			-- Echo('CHECKING DEF #' .. i, allValid.n)
			if debugMe then
				Echo('Checking '..(comment or '').."['AND'] #" .. i .. ' , current found', current_n)
			end
			FindUnits(def, allValid, pool, (comment or '') .. "['AND'] #" .. i .. ' ')
		...
		if debugMe then
			Echo('[AND] defs checked, ' .. (found and 'Found (' .. current_n..')' or 'Not Found') .. '.')
		end
		return found, allValid
	end

	local CheckAND = CheckAND
	function ProcessAND(firstTime, defs, finalValid, pool, comment, debugMe)
		local new = false
		local allValid
		if firstTime then
			-- if nothing found yet, both must match
			allValid = {n = 0, byID = {}}
			local found = CheckAND(defs, allValid, pool, comment, debugMe)
			if found then
				new = true
				if finalValid then
		...
		elseif finalValid then
			-- if already found, any match adds up
			-- Echo('N', n)
			if debugMe then
				Echo('Checking [AND] already got ' .. finalValid.n .. ', any new match adds up.')
			end
			local isnew
			for i, def in ipairs(defs) do
				_, isnew = FindUnits(def, finalValid, pool, (comment or '') .. " #" .. i .. ' ', multiCheck)
				new = new or isnew
			end
			if debugMe then
				Echo('End of checking [AND], final found: ' .. finalValid.n)
			end
		end
		return new, allValid -- created allValid only when firstTime else finalValid is updated (but it must has been given)
	end
	...
	FindUnits = function(defs, ret, altUnits, comment, multiCheck)
		...
		local processedANDs
		if multiCheck then
			processedANDs = call.groups.processedANDs
		end
					...
					if call.anyBP or (select(5, spGetUnitHealth(id)) or 0)>0.8 then  -- almost finished
						if multiCheck then
							for i, thoseDefs in ipairs(defs) do
								...
								if thoseDefs.AND then
									if processedANDs[thoseDefs.AND].byID[id] then
										pass = true
									end
								elseif CheckRequirements(thoseDefs, id) then
									pass = true
								end
								if pass then
							 		multiCheck = false
							...
							if processedANDs and defs.AND then -- only used when multiCheck has been resolved (groups) once defs has been set above, (AND is processed differently in case of switch or in case of basic defs)
								if processedANDs[defs.AND].byID[id] then
									pass = true
								end
							else
	...
	function IsIdenticalToCurrentSel(units)
		local currentSel = currentSel
		local identical = currentSel[n] and not currentSel[n+1]
		if identical then
			local byID = units.byID
			for i, id in ipairs(currentSel) do
				if not byID[id] then
					identical = false
					break
	...
	local IsIdenticalToCurrentSel = IsIdenticalToCurrentSel
	...
	ProcessFiltering = function(finalValid, defs, switch, pool, comment, multiCheck)
		...
		if defs then
			if defs['AND'] then
				new = ProcessAND(n == 0, defs.AND, finalValid, pool, comment, debugMe)
			else
				-- usual case with a single set of definitions or multiCheck
				if multiCheck then -- groups
					local processedANDs = call.groups.processedANDs
					if processedANDs then
						for def, units in pairs(processedANDs) do
							ProcessAND(units.n == 0, def, units, pool, comment .. " ['AND']", true)
						end
					end
				end
				_, new, newdefs, newdefsNum = FindUnits(defs, finalValid, pool, comment .. 'DEFS ', multiCheck)
			end
			n = finalValid.n
		end
		...
		if XAND then 
			if n>0 then
				local isnew
				_, isnew = FindUnits(XAND, finalValid, pool, comment ..  'XAND ')
				new = new or isnew
			end
			defs.XAND = XAND
			n = finalValid.n
		...
		-- if defs and defs[1] == 'nothing' then 
		-- 	call.success = true
		-- 	FinishCall(false)
		-- 	finished = true
		...
		if switch then
			...
			if n > 0 and call.switch_on_identical and IsIdenticalToCurrentSel(finalValid) then
				n = 0
			end
			if n == 0 then
				...
				while n == 0 do
					...
					local comment = comment .. 'SWITCH #' .. switch.current_defs
					if swDefs['AND'] then
						--case: multiple different units must be found together
						local allValid
						new, allValid = ProcessAND(true, swDefs['AND'], false, pool, comment .. " ['AND']")
						if new then
							finalValid = allValid -- replace completely
						end
					else
						_, new, newdefs = FindUnits(swDefs, finalValid, pool, comment)
					end
					...
					if XAND then 
						if n > 0 then
							local isnew
							_, isnew, newdefs = FindUnits(XAND, finalValid, pool, comment .. ' XAND ')
							new = new or isnew
						end
						swDefs.XAND = XAND
						n = finalValid.n
...
do
	...
	local function SetDrawing(call)
		...
		if call.option_name then
			name = name..(options[call.option_name].value and ' ON' or ' OFF')
		end -- ('not' because the option hasnt been changed yet)
		...
		DRAW = {
			...
			Fade(color[4], call.fading, 5, 1, true),
			draw_pos[1], draw_pos[2],
			size,
			call.on_press,
			firstID,
		} -- the fader has to be modified in this case because call will terminate immediately without Fading In time
	end
	local function CheckLongPress()
		if longPress.key and not longPress.active then
			...
			if osclock()-g.lastPress>currentLongPressTime+g.lagFactor/3 then
	...
	local function UpdateCall()
		if call  then
			...
			elseif method == 'on_selection' then
				myUnits = spGetSelectedUnits()
				if not myUnits[1] then
					FinishCall()
					return true
				end
			elseif method == 'cylinder' then
				if useScreenCircle then
					myUnits = Spring.Utilities.GetUnitsInScreenCircle(mx, my, g.screenradius, myTeamID, true) 
				else
			...
			if noStructure then
				...
				for i = 1, myUnits.n do
					...
					if not Units[id] or	 Units[id].isStructure then
						remove(myUnits, i)
						off = off-1
					elseif not spValidUnitID(id) or spGetUnitIsDead(id) then
						remove(myUnits, i)
						Spring.PlaySoundFile(tickSound, 0.95, 'ui')
						Echo('found invalid at method', id, spGetUnitIsDead(id), Units[id])
						off = off-1
					end
				end
							...
							if switch_group then
								...
								if testing_defs[1] then
									...
									if alt_new then
										...
										local checked = {}
										g.validUnits.checked = checked
										for id in pairs(g.validUnits.byID) do checked[id] = true end
										n, new = alt_n, alt_new
										newdefs = alt_defs
			...
			if new and n>0 then
				...
				if prev then
					...
					if g.Type then -- choose a type and narrow down the pool to this type, the choice might change if it had to pick a type from prev before
						...
						local aborted, _
						pool, _, aborted = PickType(g.Type, pool, prev)
						if aborted then
							return call
						end
						n = pool.n
					...
					if Acquire(pool, want, prev, false) then
						return call
					end
					...
					n = pool.n
					local want = GetWantN(pool, n, method)
					if Acquire(pool, want, false, false) then
						return call
					end
				end
				...
				if call.proximity and g.acquired.n>0 then -- TODO: need improved to set between picking -at- proximity of acquired and picking -along- the already acquired
					...
					for id in pairs(g.acquired.byID) do
						...
						if Acquire(prox, false, false) then
							return call
						end
					end
				end
			...
			if call.early and not call.pressed then
				call.pressed = true
		 		SelectUnits(g.acquired, call.shift)
			 	if call.give_order then
	...
	function Process(--[[delta--]]) 
		...
		-----
		local delta = --[[delta or--]] spGetLastUpdateSeconds()
	...
	--[[	-- detect Enter Key in case of Macro edition (overriden by chat)
		...
		if g.gotNewCall  then -- trigger of a new call (validated combo)
			...
			if call then
				-- Echo("call.name, osclock() is ", call.name, osclock(), 'delta', delta)

				if call.high_lock_call then
					call.high_locked = true
				end
				...
				if call.same_transported_state then
					...
					for i, def in ipairs(alldefs) do
						for i, prop in ipairs(def) do
							if type(prop)== 'string' and prop:find('isTransported') then
								table.remove(def, i)
								break
							end
						end
				...
				if call.same_transporting_state then
					...
					for i, def in ipairs(alldefs) do
						for i, prop in ipairs(def) do
							if type(prop)== 'string' and prop:find('isTransporting') then
								table.remove(def, i)
								break
							end
						end
				...
				if call.groups then
					for i, def in ipairs(call.groups) do
						if def.AND then
							call.groups.processedANDs = call.groups.processedANDs or {}
							call.groups.processedANDs[def.AND] = {n = 0, byID = {}}
						end
					end
				end
				...
				if off_sel then
					if g.Selections[1] and g.Selections[g.Selections.n+off_sel] then
						...
						SelectUnits(g.Selections[g.Selections.n], call.shift)
				...
				g.using_default = false
				g.temp = {}
				if call.on_empty_selection and not currentSel[1] then
					FinishCall(false)
					return true
				end
...
function widget:Update(dt)
	...
	----- special case of mod zero wars
	if showUnitsTab then
		if IM then
			IM.options.tab_units:OnChange()
			IM.options.tab_units:OnChange()
			showUnitsTab = false
		end
	end
	...
	function widget:KeyRelease(key, mods, keyset) -- key release can also be triggred by MouseRelease which in turn might be triggred by CheckClick
		...
		if ctrlGroups.selecting then
			...
			if toBeSelected then
				...
				SelectUnitMap(toBeSelected.units)
				ret = true
			end
		end
		...
		g.gotNewCall = true
		if call	 then
			if call.on_delay and osclock()-call.clock<call.on_delay then
				return ret
			end
			-- Echo(call.locked, "call~= HKCombos:Find(cckeys) is ", call~= HKCombos:Find(cckeys))
			if not call.locked and call~= newcall then
				if call.option_name then
					TerminateCall()
				else
					-- Process(0) -- this can terminate the call in some rare occasion (panning)
					if call then
...
do
	...
	function widget:MouseWheel(up, value)
		if not Screen0 and WG.Chili then
			Screen0 = WG.Chili.Screen0
		end
		if Screen0 then
			local mx, my = spGetMouseState()
...
--[[		----debugging all active command
		...
		if (not macro or not next(macro.keys)) and spGetModKeyState() then
			...
			-- if lastCall then
			-- 	Echo("lastCall.runs is ", lastCall and lastCall.name, lastCall and lastCall.runs, 'lastCall.shortTimed', lastCall.shortTimed, lastCall.isShortPressVersion)
			-- end

			-- not fullSel , lastCall --[[and lastCall.on_press--]] and (lastCall.method == 'cylinder' or lastCall.method == 'around_selected') and osclock()-lastCall.clock<5			
			-- lastCall --[[and lastCall.on_press--]] and (lastCall.method == 'cylinder' or lastCall.method == 'around_selected') and osclock()-lastCall.clock<5 then
			...
			if fullSel then -- reduce/re-augment the selection, from closest of mouse
				...
				local sortByDist = function(a, b)
					if not dists[a] then
						...
						dists[a] = ((ax-x)^2+(az-z)^2)
					end
					if not dists[b] then 
						local bx, by, bz = spGetUnitPosition(b)
						dists[b] = ((bx-x)^2+(bz-z)^2)
					end
				...
				if not fullSel.defIDs then
					...
					for defID, units in pairs(fullSel.defIDs) do
						for i, id in ipairs(units) do
							if spGetUnitIsDead(id) or not spValidUnitID(id) then -- never happened ??
								Echo('sorted selection contain dead/invalid unit !', id, ' Units[id]?', Units[id])
							end
				...
				DRAW = {declare = true
				   	 ...
				   	 , ('Resize Selection %d%%'):format(fullSel.partial * 100)
				   	 , false
				   	 , 1, 1, 1, 0.8
				   	 , Fade(0.8, 0.5, 5, 1, true)
				   	 , Fade(0.8, 0.5, 5, 1, true)
				   	 -- , DEFAULT_DRAW_POS[1]-200, DEFAULT_DRAW_POS[2]
				   	 , DEFAULT_DRAW_POS[1], DEFAULT_DRAW_POS[2]
				   	 , 25
				   	 , true}
				...
				return true
			elseif not fullSel and lastCall and (lastCall.shortTimed or lastCall.runs <= 1) and (lastCall.method == 'cylinder' or lastCall.method == 'around_selected') and osclock()-lastCall.clock<5 then
				macro = lastCall
		...
		if macro then
			...
			if up then
				...
				g.radius = max(40, g.radius*(1+(1/11)*value))		
				g.screenradius = g.radius / (SCREEN_RATIO * screen_ratio)
			end
			-- ratio = max(ratio, 1)
			-- local ratio = (GetCameraHeight(spGetCameraState()) * 1.5 / 1000)
	...
	function widget:CommandsChanged(force)
		...
		--Echo("round(osclock()*10)-lastTime is ", round(osclock()*10)-lastTime)
		-- chk('reset')
		-- local lastCall = last.call
		-- if lastCall then
		...
		if zerowars then -- show the units tab on selected fac in ZeroWars
			for i, id in pairs(newsel) do
				local unit = Units[id]
				if not unit then
					Echo('unit '.. id ..' is not registered (' .. widget.GetInfo().name ..')')
				elseif unit.isFakeFac then
					-- Echo('2')
					if IM then
						showUnitsTab = true
					end
		...
		-- local thistime = round(osclock()*10)
			...
			if DRAW.drawing then
				...
				if DRAW.finishing then
					if dName == 'Plus' or dName == 'Minus' then
						...
						-- Echo("noFadeIn is ", noFadeIn)
						dFader(0.1, noFadeIn and 20 or 100, -1) -- give lesser amplitude when 
				...
				glLineStipple(true)
				if DRAW.circle and not useScreenCircle then
					glLineWidth(1.5)
					-- glPushMatrix()
	...
	--[[			for c in pairs(Circles) do
		...
		function widget:DrawScreen()
			...
			if dbgRadius and call and call.radius then
				...
				-- glText('radius: ' .. tostring(g.radius) .. '-' .. tostring(call and call.radius or '') .. '\nscreen radius: ' .. tostring(g.screenradius) ,resX/2,resY/3, 15, 'cno')
				glText(txt ,resX/2,resY/3, 15, 'cno')
			end
			if DRAW.drawing then
				if DRAW.circle and useScreenCircle then
					local mx, my = spGetMouseState()
					glColor(dR, dG, dB, dAlpha)
					glLineStipple(true)
					glLineWidth(1.5)
					gluDrawScreenCircle(mx, my, g.screenradius)
					glLineWidth(1)
					glColor(1, 1, 1, 1)
				end
				...
				if sh.sw then
	   			...
	   			glText(
	   				string.format('%s%s%s',
	   					dSelName or dName or '??',
	   					extra,
	   					(sh.Sel_n>0 and ' ('..sh.Sel_n..')' or ''),
	   					(g.hasPrefered and ' #' .. g.hasPrefered or ''),
	   					(sh.SelCost~= 0 and  '\n($'..sh.SelCost..')' or '')
	   				)
	   				, dPosX, dPosY, dSize, 'c'
	   			)
...
function widget:Shutdown()
	...
	if WG.UnitsIDCard then WG.UnitsIDCard.subscribed[widget:GetInfo().name] = nil end
	WG.EzSelecting = false
end
