...
return
{
  ...
  // #endif
     ...
     vec4 diffuse     = texture2D(textureS3o2, gl_TexCoord[0].st) * teamColor ;
     // diffuse = mix(diffuse*0.1, teamColor, diffuse.g/3);
     // diffuse = teamColor - diffuse.g;
     diffuse =  teamColor * 0.15 + diffuse.g*0.2 + ((1-diffuse.g) * teamColor)*0.2;
     // diffuse = teamColor * (diffuse.g *);
   //   vec4 extraColor  = texture2D(textureS3o1, gl_TexCoord[0].st);
		 // // extraColor =  vec4(0.4,0.4,0.4,0.4);
   //   vec3 reflectDir = reflect(cameraDir, normal);
   //   vec3 specular   = textureCube(specularTex, reflectDir).rgb * extraColor.g * 4.0;
   //   specular = extraColor.g * 4;
   ...
   //   float shadow = GetShadowCoeff(gl_TexCoord[1] + vec4(0.0, 0.0, -0.00005, 0.0));
     ...
     // gl_FragColor.a   = extraColor.a * tint.a ;
  }
