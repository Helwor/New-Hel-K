...
local iconFrameTex = setmetatable(
	{},
	{ 
		__index = function(self, udef)
...
local function WriteTable(concatArray, tab, tabName, params)
	...
	for i=0,endTab do
		local v = tab[i]
		if v then
			ProcessKeyValuePair(i,v, (tab[0] == nil), (not isDict) and i == endTab)
			processed[i] = true
		end
	end
	for i,v in pairs(tab) do
...
local spGetUnitDefID = Spring.GetUnitDefID
local spGetSelectUnitSorted = Spring.GetSelectedUnitsSorted
function widget:SelectionChanged(units)
	if not (units and units[1]) then
		WG.selectionEntirelyCons = false
		return
	end
	for defID in pairs(spGetSelectUnitSorted()) do
		if not builderDefs[defID] then
			WG.selectionEntirelyCons = false
			return
		end
	end
	-- for i = 1, #units do
	-- 	if not builderDefs[spGetUnitDefID(units[i])] then
	-- 		WG.selectionEntirelyCons = false
	-- 		return
	-- 	end
	-- end
...
Spring.OriIsGUIHidden = Spring.IsGUIHidden
function Spring.IsGUIHidden()
	return guiHidden
end
function widget:Update()
	guiHidden = OriIsGUIHidden()
end
----------------------------------------------------------
