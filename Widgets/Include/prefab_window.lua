...
function WINDOW:Create(children, minize)
    ...
    --     children = children,
    -- }
    self.checkControl = WG.Chili.Checkbox:New{
        caption = 'check me',
        tooltip = 'or not',
    ...
    self.extraButton = WG.Chili.Button:New{
        caption = "button",
        OnClick = { function() Spring.SendCommands("chat","watsup") end },
        --backgroundColor=color.sub_close_bg,
        --textColor=color.sub_close_fg,
    ...
    self.header = WG.Chili.Label:New{
        caption = '- header -',
    ...
    self.scrollPanel:AddChild(self.header)
    ...
    self.win = {
        ...
        caption = 'Win Custom #' .. self.count,
        name = 'win_custom_id_' .. self.id, -- creating a window that have the same name and an existing one will result in an auto dispose
        minWidth = 250,
        minHeight = 400,
        height = 28,
        OnDispose = { 
