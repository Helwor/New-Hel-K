...
local function AddNodeFunctions()
	...
	function TreeViewNode:ClearChildren()
		...
		if not(self.root) then
			...
			if caption then
				local len = #children
				local last = children[len]
				-- Echo("len is ", len)
				children[1] = last
				children[len] = nil
				children[last] = 1 -- fix indexing
				children[caption] = nil -- fix indexing
			end
		end
		...
		if caption then
			children[1] = caption
			children[caption] = 1  -- fix indexing
		end
	end
...
local function AddFocusControl()
	if not Chili.Screen0.FocusControl then
...
function widget:Initialize()
	if not WG.Chili then
		local msg = sig .. 'Chili is required, shutting down...'
		Echo(msg)
		widget.status = msg
