...
function widget:UnitUnloaded(unitID, unitDefID, unitTeam, transID, transportTeam)
	...
	-- fixing CMD_RAW_BUILD breaking the construction if the unit is unloaded at reach of construction
	if cmd == CMD_RAW_BUILD then
		local queue = spGetCommandQueue(unitID, 2)
		local resend
		for i, order in ipairs(queue) do
			spGiveOrderToUnit(unitID, CMD_REMOVE, order.tag)
			if i == 2 then
				resend = order
			end
		end
		if resend then
			spGiveOrderToUnit(unitID, CMD_INSERT, {0, resend.id, resend.options.coded, unpack(resend.params)}, CMD_OPT_ALT)
		end
	end
