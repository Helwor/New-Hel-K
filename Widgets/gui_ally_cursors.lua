...
--
-- Helwor: delay the cursor position to get synchrone with the server replies
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
...
function widget:RecvLuaMsg(msg, playerID)
  ...
  then
    ...
    if (l==numMousePos) then
      ...
      local now = clock()
      if acp then
        -- history last collected x and z is pushed from first places to 5 and 6th
        acp[(numMousePos)*2+1]   = acp[1] 
        acp[(numMousePos)*2+2]   = acp[2]

        for i=0,numMousePos-1 do
          -- newposes are added at 1,2,3 & 4
          acp[i*2+1] = newPos[i*2+1]
          acp[i*2+2] = newPos[i*2+2]
        end
        -- pos 7 and 8 indicate time and a left or right click
        acp[(numMousePos+1)*2+1] = now
        ...
        -- newpos 1 and 2 is also added at 5 and 6
        ...
        local ping
        -- add teamID at #9
        _,_,_,acp[(numMousePos+1)*2+3],_, ping = GetPlayerInfo(playerID, false)
        -- acp.delay = now + ping*2
      end
    end
    return true
  end
...
function widget:Update(t)
  ...
  -- every 0.2 sec record the mouse pos and mouse press at 0,1 - 2,3 ...
  ...
  if (updateTimer>sendPacketEvery)and(n>=numMousePos) then
    ...
    for i=numMousePos,1,-1 do
      -- extract 4, 5, 2, 3 from poshistory and include a current mousepress to be sent
...
-- local delayToPing = true
-- local DELAYED = {}
local function draw(teamID, n, wx,gy,wz, pressed)
  if (IsSphereInView(wx,gy,wz,16)) then
    -- local r,g,b = GetTeamColor(teamID)
    local color
    if (pressed) then --mouse pressed?
      glColor(1,0,0,n*0.2)
    else
    ...
    OrderDraw(teamID,n,wx,gy,wz) -- draw on top of icons with a fixed size on screen to watch zoomed out
  end
end
function widget:DrawWorldPreUnit()
  ...
  for playerID,data in pairs(WG.alliedCursorsPos) do
    local len = #data
    local teamID = data[len]
    local time = data[len-2]
    local pressed =  data[len-1]
    for n=0,5 do
      local wx,wz = data[1],data[2]
      local lastUpdatedDiff = now-time + n*0.025
      ...
      draw(teamID, n, wx,gy,wz, pressed)
      -- if (IsSphereInView(wx,gy,wz,16)) then
      --   -- local r,g,b = GetTeamColor(teamID)
      --   local color
      --   if (data[len-1]) then --mouse pressed?
      --     glColor(1,0,0,n*0.2)
      --   else
      --     SetTeamColor(teamID,n*0.2)
      --   end
      ...
      -- end
      -- if data.delay then
      --   DELAY[teamID] = DELAY[teamID] or {}
      --   table.insert(DELAY[teamID], {n, wx,gy,wz, pressed})
      -- end
