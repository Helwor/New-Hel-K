...
local function CorrectLockpointCentering(cs, lockPoint) --For COFC's zoom out to center setting
	...
	local wcx, wcy, wcz = cs.px + dirx, cs.py + diry, cs.pz + dirz
	lockPoint.worldCenter = {x = wcx, y = wcy, z = wcz}
	-- lockPoint.worldCenter = {x = cg[1], y = cg[2], z = cg[3]}

	local minX, minZ, maxX, maxZ = GetCenterBounds(cs)
	local cdx, cdz = 0, 0
	-- local corrected = false
	if wcx < minX then cdx = minX - wcx ; wcx = minX end
	if wcx > maxX then cdx = maxX - wcx ; wcx = maxX end
	if wcz < minZ then cdz = minZ - wcz ; wcz = minZ end
	if wcz > maxZ then cdz = maxZ - wcz ; wcz = maxZ end
	-- if corrected then
		-- lockPoint.world.x = wcx
		-- lockPoint.world.z = wcz
	-- end
	-- Spring.Echo("lockPoint.world: {"..lockPoint.world.x..", "..lockPoint.world.z.."}")
	-- Spring.Echo("Bounds: x: "..minX.." - "..maxX..", z: "..minZ.." - "..maxZ..", lockPoint.worldCenter: {"..wcx..", "..wcz.."}")
	return cdx, cdz
end
