...
options = {
	...
	maxzoomout_alt = {
		...
		-- must be updated/update into the code, must adapt to FOV change
		desc = '% of the average h/w map fitting screen if Alt is pressed',
		type = 'number',
		min = 0.5, max = 3, step = 0.01,
...
local function RotateCamera(x, y, rdx, rdy, smooth, lock, tilt, alt)
	local cs = GetTargetCameraState()
	-- local cs1 = cs
	lastMouseX = nil
	if cs.rx then
...
function widget:Update(dt)
	if rotate  or springscroll then
...
function widget:MouseMove(x, y, dx, dy, button)
	...
	if rotate then
		...
		oldDx = oldDx*0.6 + dx*0.4
		oldDy = oldDy*0.6 + dy*0.4
		if abs(oldDx) > 1 or abs(oldDy) > 1 then
			local alt = spGetModKeyState()
			RotateCamera(x, y, oldDx, oldDy, true, ls_have, false, alt)
		end
		
		Spring.WarpMouse(x-dx, y-dy)
...
local function FlipCamera()
	...
	local cs = GetTargetCameraState()
		...
		cs.ry = cs.ry + PI * (cs.ry < 0 and 1 or - 1)
	if (ls_onmap or options.freemode.value) then --if have lock (ls_have ==true) and is either onmap or freemode (offmap) then update camera properties.
		local cstemp = UpdateCam(cs)
		if cstemp then
			cs = cstemp;
		else
